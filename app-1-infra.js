...
async function shareCanvasImage(cv,nombre,titulo,textoGuardada){
  if(_shareBusy)return;                                   // un segundo toque no arranca otro intento
  _shareBusy=true;
  ...
  try{
    ...
    if(navigator.canShare&&navigator.canShare({files:[file]})){
      ...
      try{ await navigator.share(pay); }
        ...
        if(n==='NotAllowedError'){ if(typeof toast==='function')toast('Toca «Compartir» otra vez'); return; }
        throw e;
      }
      return;
    }
...
// v500: e229, e250 y e251 vuelven con foto porque el PO mandó FOTOS Y VIDEOS REALES de esas
// máquinas en su gimnasio, y con una referencia de verdad el generador sí las reproduce.
// Es lo que ya decía el pipeline del catálogo: cada una de las 219 partía de una foto previa.
['e228','e229','e230','e241','e242','e243','e244','e245','e246','e247','e248','e249','e250','e251'].forEach(id=>EX_IMG_IDS.add(id));
...
const defaultExercises=[
  ...
  {id:'e136',name:'Caminata del Granjero (Farmers Walk)',muscle:'otro',type:'Funcional',sets:3,reps:40,icon:'🧳',env:['casa','parque','gym'],desc:'Carga pesada en cada mano y camina erguido: fuerza de agarre, trapecio, core y postura bajo carga real — el ejercicio funcional más transferible a la vida diaria. Hombros atrás, pasos cortos y firmes, sin ladearse. Se cuentan PASOS, no repeticiones (o metros, si prefieres medir la distancia). Que el agarre sea el límite, no la postura.',descSimple:'Toma una mancuerna o pesa en cada mano, ponte derecho con los hombros atrás y camina dando pasos cortos y firmes, sin inclinarte a ningún lado. Lo que cuentas son pasos: la app te pide PASOS, no repeticiones. Suelta el peso cuando ya no puedas mantener la postura, no cuando se te abran las manos. Trabaja agarre, abdomen y postura a la vez.',muscleLabel:'Agarre, core y cuerpo completo',ytQuery:'caminata del granjero farmers walk tutorial'},
  ...
  // ── LOTE DE MÁQUINAS (e228-e249, 2026-08-18) ─────────────────────────────────────────
  // Nace de una queja del PO con la razón de su lado: «he estado en otros gyms y veo máquinas que
  // no están en nuestra biblioteca». Medido contra el piso de un gimnasio de cadena (Smart Fit /
  // Bodytech / Stark, líneas Matrix-Life Fitness): **faltaban 25 de las 30 estaciones revisadas**,
  // y el catálogo tenía 36 ejercicios de máquina contra 127 de peso corporal — una biblioteca de
  // casa en una app cuyos asesorados entrenan en gimnasio.
  // TODOS declaran `env:['gym']` (el hueco de los 95 sin `env` no se agranda) y TODOS llevan nivel
  // explícito en EX_LEVEL: un id sin nivel cae a 'I' y un principiante no lo recibe JAMÁS.
  {id:'e228',name:'Press Inclinado en Máquina',muscle:'pecho',type:'Compuesto',sets:4,reps:10,icon:'🏗️',env:['gym'],desc:'Empuje inclinado guiado: carga el pectoral SUPERIOR, que es la zona que más se queda atrás cuando solo se hace banca plana. La trayectoria fija deja concentrarse en empujar sin pelear con el equilibrio, así que admite más carga con menos riesgo. Omóplatos apoyados y codos a unos 45°, sin dejar que el hombro se vaya adelante abajo.',descSimple:'Siéntate con la espalda pegada al respaldo y empuja los agarres hacia arriba y adelante. Baja despacio hasta sentir el pecho estirado, sin que los codos pasen muy por detrás del cuerpo. Es el hermano del press inclinado con barra, pero la máquina te lleva el camino.',muscleLabel:'Pecho superior, hombro y tríceps',ytQuery:'press inclinado en maquina tecnica'},
  {id:'e229',name:'Remo con Apoyo Pectoral en Máquina',muscle:'espalda',type:'Compuesto',sets:4,reps:12,icon:'🪑',env:['gym'],desc:'Remo horizontal con el pecho apoyado: el apoyo ANULA la carga sobre la lumbar, así que el jalón lo hace la espalda media y no la cadera. Es el remo más seguro del gimnasio y el primero que debería aprender alguien con molestia lumbar. Codos pegados al cuerpo y omóplatos juntándose al final.',descSimple:'Siéntate con el pecho contra el respaldo y jala los agarres hacia atrás llevando los codos junto al cuerpo. Aprieta los omóplatos al final y vuelve despacio. Como el pecho va apoyado, la espalda baja no trabaja: es ideal si te molesta la cintura.',muscleLabel:'Espalda media, dorsal y bíceps',ytQuery:'remo con apoyo pectoral maquina tecnica'},
  {id:'e230',name:'Remo en T (T-Bar Row)',muscle:'espalda',type:'Compuesto',sets:4,reps:10,icon:'🇹',env:['gym'],desc:'Remo con barra en T: permite cargar mucho con un agarre neutro cómodo y un recorrido largo para el dorsal. Bisagra de cadera con la espalda recta y el pecho arriba; el tirón termina llevando los codos hacia atrás, no subiendo los hombros. Exige control lumbar, por eso no es de entrada.',descSimple:'Párate a horcajadas sobre la barra, dobla la cadera con la espalda recta y jala el agarre hacia el ombligo. Baja el peso con control sin redondear la espalda. Si sientes la cintura, baja el peso: primero la postura.',muscleLabel:'Dorsal, espalda media y bíceps',ytQuery:'remo en t barra tecnica correcta'},
  {id:'e231',name:'Dominadas Asistidas en Máquina',muscle:'espalda',type:'Compuesto',sets:4,reps:10,icon:'🅰️',env:['gym'],desc:'La máquina descuenta parte de tu peso para que puedas hacer dominadas ANTES de poder con el peso completo: es el camino real hacia la dominada, no un sustituto. Cuelga con los hombros activos (sin encogerlos), jala llevando los codos al costado y baja controlando. A menos asistencia, más difícil.',descSimple:'Arrodíllate o párate en la plataforma y elige el peso de ayuda: cuanto MÁS peso pongas, más fácil. Jala tu cuerpo hasta pasar la barbilla y baja despacio. Cada semana quítale un poco de ayuda: así llegas a la dominada de verdad.',muscleLabel:'Dorsal, espalda media y bíceps',ytQuery:'dominadas asistidas maquina tecnica'},
  {id:'e232',name:'Pullover en Máquina',muscle:'espalda',type:'Aislamiento',sets:3,reps:12,icon:'🌙',env:['gym'],desc:'Aislamiento del dorsal en máquina: el brazo hace de palanca larga y el dorsal trabaja sin que el bíceps se lleve el trabajo, que es lo que pasa en los jalones. Codos semirrígidos durante todo el arco y el tronco quieto contra el respaldo.',descSimple:'Siéntate en la máquina y empuja los agarres hacia abajo y adelante con los brazos casi estirados, hasta sentir la espalda apretarse. Vuelve despacio dejando que la espalda se estire. El bíceps casi no participa: aquí manda la espalda.',muscleLabel:'Dorsal y pecho',ytQuery:'pullover en maquina tecnica'},
  {id:'e233',name:'Elevaciones Laterales en Máquina',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'🕊️',env:['gym'],desc:'Elevación lateral guiada: la máquina fija el codo y la trayectoria, así que el deltoides LATERAL trabaja sin que el cuerpo se balancee ni el trapecio se meta. Es la variante más fácil de aprender y la que permite subir carga sin perder la técnica.',descSimple:'Siéntate con los codos contra los apoyos y sube los brazos hacia los lados hasta la altura del hombro. Baja despacio. No encojas el cuello: los hombros van abajo y lo que sube son los codos.',muscleLabel:'Hombro lateral',ytQuery:'elevaciones laterales en maquina tecnica'},
  {id:'e234',name:'Curl de Bíceps en Máquina',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['gym'],desc:'Curl con el brazo apoyado en máquina: el apoyo impide el balanceo, así que el bíceps recibe todo el trabajo en el rango completo. La resistencia guiada mantiene tensión también al bajar, que es la fase donde más se construye.',descSimple:'Apoya la parte de atrás de los brazos en el cojín y sube el agarre doblando los codos. Aprieta arriba y baja despacio hasta casi estirar. No despegues los codos del apoyo ni uses la espalda.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en maquina tecnica'},
  {id:'e235',name:'Extensión de Tríceps en Máquina',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔧',env:['gym'],desc:'Extensión de codo guiada: aísla las tres cabezas del tríceps sin exigirle estabilidad al hombro ni a la muñeca, que es lo que limita al press francés. Muy útil para cargar el tríceps cuando el hombro está sensible.',descSimple:'Siéntate con la espalda apoyada y estira los codos empujando el agarre hacia abajo o adelante, según la máquina. Vuelve despacio sin dejar que el peso te lleve. Los codos se quedan quietos: solo se abre y se cierra el antebrazo.',muscleLabel:'Tríceps',ytQuery:'extension de triceps en maquina tecnica'},
  {id:'e236',name:'Prensa Horizontal Sentada',muscle:'piernas',type:'Compuesto',sets:4,reps:12,icon:'🛗',env:['gym'],desc:'Prensa de pierna sentada (horizontal): mismo empuje que la de 45° pero con la espalda vertical y menos flexión de cadera, así que es más cómoda para quien tiene molestia lumbar o poca movilidad. Pies a la anchura de la cadera, rodillas siguiendo la punta del pie, sin bloquear del todo arriba.',descSimple:'Siéntate con la espalda bien apoyada y empuja la plataforma con toda la planta del pie. Vuelve despacio hasta que las rodillas queden a unos 90° y empuja otra vez. No estires del todo las rodillas al final.',muscleLabel:'Cuádriceps y glúteo',ytQuery:'prensa horizontal sentada tecnica'},
  {id:'e237',name:'Elevación de Talones de Pie en Máquina',muscle:'piernas',type:'Aislamiento',sets:4,reps:15,icon:'🦵',env:['gym'],desc:'Gemelo de pie en máquina: con la rodilla estirada el trabajo cae sobre el GEMELO (gastrocnemio), que es el que da el volumen de la pantorrilla. Rango completo — abajo estirando y arriba de puntillas con pausa — porque este músculo responde al recorrido, no al rebote.',descSimple:'Ponte bajo los apoyos con la punta de los pies en el escalón y sube de puntillas lo más alto que puedas. Aguanta arriba un segundo y baja despacio hasta sentir el estiramiento. Sin rebotar.',muscleLabel:'Gemelos',ytQuery:'elevacion de talones de pie en maquina'},
  {id:'e238',name:'Elevación de Talones en Prensa',muscle:'piernas',type:'Aislamiento',sets:4,reps:15,icon:'🛗',env:['gym'],desc:'Gemelo hecho en la prensa de pierna: permite cargar mucho sin comprimir la columna, porque el peso va a los pies y no a los hombros. Solo la punta del pie apoyada en la plataforma y las rodillas casi estiradas (sin bloquear) para que trabaje el gemelo.',descSimple:'En la prensa, baja el peso hasta apoyar solo la punta de los pies en el borde de la plataforma. Empuja con los dedos estirando el tobillo y vuelve despacio dejando que el talón caiga. Las rodillas se quedan casi rectas.',muscleLabel:'Gemelos',ytQuery:'gemelos en la prensa 45 grados tecnica'},
  {id:'e239',name:'Patada de Glúteo en Máquina',muscle:'gluteo',type:'Aislamiento',sets:3,reps:15,icon:'🍑',env:['gym'],desc:'Extensión de cadera guiada para el glúteo mayor: el apoyo del tronco evita que la lumbar arquee para "ayudar", que es el error clásico de la patada en polea. Empuje corto y apretando arriba, sin buscar rango de más.',descSimple:'Apóyate en el pecho y empuja la plataforma hacia atrás con una pierna, apretando el glúteo al final. Vuelve despacio. No arquees la espalda para llegar más lejos: el movimiento sale de la cadera.',muscleLabel:'Glúteo mayor',ytQuery:'patada de gluteo en maquina tecnica'},
  {id:'e240',name:'Crunch en Máquina Abdominal',muscle:'core',type:'Aislamiento',sets:3,reps:15,icon:'🧱',env:['gym'],desc:'Flexión de tronco con carga graduable: permite progresar en el abdomen igual que en cualquier otro músculo, cosa que el crunch en suelo no deja hacer. El movimiento es ACORTAR el abdomen (llevar las costillas a la cadera), no doblar la cadera ni tirar del cuello.',descSimple:'Siéntate y agarra los apoyos; baja el pecho hacia el ombligo curvando la espalda alta, sin tirar con los brazos ni con el cuello. Vuelve despacio. Piensa en cerrar el abdomen como un acordeón.',muscleLabel:'Abdomen',ytQuery:'crunch en maquina abdominal tecnica'},
  {id:'e241',name:'Abdominales en Banco Declinado',muscle:'core',type:'Aislamiento',sets:3,reps:15,icon:'📐',env:['gym'],desc:'Crunch en banco declinado: la inclinación añade resistencia al principio del recorrido, donde el abdomen es más fuerte. Sube curvando la espalda alta y no tirando del cuello; cuanto más declinado, más difícil. No es de entrada porque exige controlar la lumbar.',descSimple:'Engancha las piernas en el banco inclinado y sube el tronco curvando la espalda, con las manos en el pecho. Baja despacio sin dejarte caer. Si sientes la espalda baja, reduce la inclinación.',muscleLabel:'Abdomen',ytQuery:'abdominales en banco declinado tecnica'},
  {id:'e242',name:'Press de Banca en Multipower',muscle:'pecho',type:'Compuesto',sets:4,reps:10,icon:'🔩',env:['gym'],desc:'Banca en máquina Smith: la barra va guiada, así que se puede empujar fuerte sin necesitar quien vigile ni dominar el equilibrio. Es la vía para que alguien nuevo aprenda el patrón de empuje horizontal con carga real. Omóplatos juntos y la barra bajando a la línea del pecho medio.',descSimple:'Acuéstate bajo la barra guiada, desbloquéala girando las muñecas y bájala hasta el pecho. Empuja hacia arriba sin despegar la espalda del banco. Como la máquina sostiene la barra, puedes ir más pesado con seguridad.',muscleLabel:'Pecho, hombro y tríceps',ytQuery:'press de banca en multipower smith tecnica'},
  {id:'e243',name:'Hip Thrust en Multipower',muscle:'gluteo',type:'Compuesto',sets:4,reps:12,icon:'🔩',env:['gym'],desc:'Empuje de cadera con la barra guiada del Smith: quita el problema de montar y cuadrar la barra del hip thrust libre y deja subir carga rápido, que es lo que este ejercicio pide. Barbilla metida, costillas abajo y extensión COMPLETA de cadera apretando el glúteo arriba, sin arquear la lumbar.',descSimple:'Siéntate en el suelo con la espalda alta apoyada en un banco y la barra guiada sobre la cadera (con la almohadilla). Empuja el piso con los talones y sube la cadera hasta quedar recta. Aprieta el glúteo arriba y baja despacio.',muscleLabel:'Glúteo mayor e isquios',ytQuery:'hip thrust en multipower smith tecnica'},
  {id:'e244',name:'Remo en Multipower',muscle:'espalda',type:'Compuesto',sets:4,reps:12,icon:'🔩',env:['gym'],desc:'Remo con barra guiada: el recorrido fijo permite concentrarse en llevar los codos atrás sin pelear con la barra. Bisagra de cadera con la espalda recta y el tronco casi paralelo al suelo; la barra sube al ombligo, no al pecho.',descSimple:'De pie frente a la barra guiada, dobla la cadera con la espalda recta y jala la barra hacia el ombligo llevando los codos atrás. Baja con control. Si te molesta la cintura, sube un poco el tronco.',muscleLabel:'Dorsal, espalda media y bíceps',ytQuery:'remo en multipower smith tecnica'},
  {id:'e245',name:'Zancada Búlgara en Multipower',muscle:'piernas',type:'Compuesto',sets:3,reps:10,icon:'🔩',env:['gym'],desc:'Búlgara con la barra guiada: la ayuda del Smith para el equilibrio deja cargar la pierna de verdad, que es lo que el ejercicio busca. Trabaja una pierna a la vez, corrige diferencias entre lados y castiga glúteo y cuádriceps con poca carga en la columna.',descSimple:'Pon un pie atrás sobre un banco y el otro adelante, bajo la barra guiada. Baja doblando la rodilla de adelante hasta que el muslo quede casi paralelo al suelo y sube empujando con el talón. Cambia de pierna.',muscleLabel:'Cuádriceps y glúteo',ytQuery:'zancada bulgara en multipower smith'},
  {id:'e246',name:'Zancada con Barra al Hombro',muscle:'piernas',type:'Compuesto',sets:3,reps:10,icon:'🏋️',env:['gym'],desc:'Zancada cargada con barra: patrón unilateral con más carga que con mancuernas y una exigencia real de estabilidad de cadera y core. Tronco erguido, rodilla de adelante siguiendo la punta del pie y la de atrás bajando hacia el suelo sin golpearlo.',descSimple:'Con la barra apoyada en la espalda alta, da un paso adelante y baja hasta que las dos rodillas queden a 90°. Empuja con el talón de adelante para volver. Mantén el pecho arriba todo el tiempo.',muscleLabel:'Cuádriceps y glúteo',ytQuery:'zancada con barra tecnica correcta'},
  ...
  {id:'e251',name:'Peso Muerto Rumano en Multipower',muscle:'gluteo',type:'Compuesto',sets:4,reps:10,icon:'🔩',env:['gym'],desc:'Bisagra de cadera con la barra GUIADA: el Smith sostiene la trayectoria, así que se puede aprender el patrón —cadera atrás, espalda recta, barra pegada a la pierna— sin pelear con el equilibrio, y cargar el isquio y el glúteo antes de dominar la versión libre. Rodillas casi rectas (no bloqueadas) y la bajada manda hasta donde la espalda se mantenga recta, ni un centímetro más.',descSimple:'De pie frente a la barra guiada, empuja la cadera hacia atrás y baja la barra rozando el muslo, con la espalda recta y las rodillas casi estiradas. Baja hasta que sientas el tirón detrás de la pierna y sube apretando el glúteo. Si la espalda se redondea, ya bajaste de más.',muscleLabel:'Isquiotibiales y glúteo',ytQuery:'peso muerto rumano en multipower smith tecnica'},
  {id:'e252',name:'Patada de Tríceps en Polea',muscle:'triceps',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Tríceps con el codo FIJO y la carga en su punto más corto: de pie frente a la polea baja, tronco inclinado, codo pegado al costado y a la altura del tronco; extiende el antebrazo hacia atrás hasta bloquear y vuelve con control. La polea mantiene tensión constante, así que el pico de resistencia queda en la extensión final y el codo nunca pasa por flexión profunda bajo carga.',descSimple:'De pie frente a la polea de abajo, inclínate un poco al frente con la espalda recta. Pega el codo a tu costado y, sin moverlo, estira el brazo hacia atrás hasta que quede recto. Vuelve despacio. Solo se mueve del codo para abajo.',muscleLabel:'Tríceps',ytQuery:'patada de triceps en polea kickback cable tutorial'},
  {id:'e253',name:'Patada de Tríceps con Banda',muscle:'triceps',type:'Aislamiento',sets:3,reps:15,icon:'🎗️',env:['casa','parque','gym'],level:'P',desc:'Misma mecánica que la patada en polea, con banda: pisa un extremo o fíjala baja, tronco inclinado, codo pegado al costado y quieto; extiende el antebrazo hacia atrás. La banda da su MÍNIMA tensión en la posición estirada y la máxima al final — la curva opuesta a la de un jalón, y por eso carga el tríceps sin meter el codo en flexión profunda.',descSimple:'Pisa un extremo de la banda y agarra el otro. Inclínate al frente con la espalda recta y el codo pegado al costado. Estira el brazo hacia atrás hasta ponerlo recto y vuelve despacio, sin mover el codo de su sitio.',muscleLabel:'Tríceps',ytQuery:'patada de triceps con banda elastica kickback'},
  {id:'e254',name:'Tríceps Isométrico Autorresistido',muscle:'triceps',type:'Isométrico',sets:3,reps:20,icon:'✊',env:['corporal','casa','parque','gym'],level:'P',desc:'Espejo del curl isométrico autorresistido (e151), para tríceps: con el codo a media flexión, una mano empuja el antebrazo hacia la extensión mientras la otra lo frena. La carga la gradúa la propia persona, así que no puede excederse — es la única forma de cargar tríceps sin equipo y sin apoyo sobre la mano.',descSimple:'Dobla un brazo a medio camino y pon la otra mano encima del antebrazo. Empuja como si quisieras estirar el brazo mientras la otra mano lo frena. Aprieta fuerte unos segundos, suelta y repite. Tú decides cuánta fuerza haces.',muscleLabel:'Tríceps',ytQuery:'triceps isometrico autorresistido sin pesas'},
  {id:'e255',name:'Curl de Bíceps en Polea con Cuerda',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Curl en polea baja con cuerda: agarre neutro que puedes abrir al final girando las muñecas hacia afuera. La polea mantiene tensión constante, así que el bíceps no descansa arriba como sí pasa con la mancuerna.',descSimple:'De pie frente a la polea de abajo con la cuerda en las manos, palmas enfrentadas. Sube doblando los codos sin moverlos del costado y baja despacio. Al final puedes abrir un poco las manos.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en polea con cuerda tecnica'},
  {id:'e256',name:'Curl de Bíceps en Polea con Barra Recta',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Curl en polea baja con barra recta: agarre supino fijo, el que más carga permite de las variantes en polea. Codos pegados al costado; el tirón constante del cable evita el punto muerto de arriba.',descSimple:'De pie frente a la polea de abajo, agarra la barra con las palmas hacia arriba. Sube doblando los codos sin despegarlos de las costillas y baja controlando. No balancees el cuerpo.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en polea con barra recta'},
  {id:'e257',name:'Curl de Bíceps en Polea con Barra Z',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Curl en polea baja con la barra corta acodada (Z): los quiebres del agarre dejan la muñeca en semisupinación y quitan la tensión que la barra recta le mete al antebrazo. Misma carga, menos molestia de muñeca y codo.',descSimple:'Igual que el curl en polea con barra, pero con la barra cortita que tiene curvas. Agárrala en las partes inclinadas: la muñeca queda más cómoda. Sube sin mover los codos y baja despacio.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en polea con barra z corta'},
  {id:'e258',name:'Curl de Bíceps en Polea a una Mano',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Curl unilateral en polea baja con agarre simple: permite igualar diferencias entre brazos y girar la mano en supinación completa al subir, que es donde el bíceps termina de contraerse.',descSimple:'Con una sola mano en la polea de abajo, sube doblando el codo y gira un poco la palma hacia arriba al final. Haz todas las repeticiones de un lado y cambia. Sirve para emparejar los dos brazos.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en polea a una mano unilateral'},
  {id:'e259',name:'Curl de Bíceps en Polea Alta a Dos Manos',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['gym'],level:'I',desc:'Curl con las poleas ARRIBA y los brazos abiertos en cruz: el bíceps trabaja en su posición más acortada y el pico de tensión cae en la contracción final, al revés que en la polea baja.',descSimple:'Párate en medio de dos poleas altas con los brazos abiertos como una cruz. Sin bajar los codos, dobla los brazos llevando las manos hacia tus orejas y aprieta. Vuelve despacio.',muscleLabel:'Bíceps',ytQuery:'curl de biceps en polea alta doble biceps'},
  {id:'e260',name:'Curl de Bíceps Bayesiano en Polea',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Curl en polea baja de espaldas al aparato, con el brazo ligeramente por detrás del cuerpo: esa posición estira la cabeza larga del bíceps antes de contraerla y es lo que lo diferencia del curl normal.',descSimple:'Dale la espalda a la polea de abajo y da un paso al frente, con el brazo un poco atrás. Sube doblando el codo sin llevarlo hacia adelante. Vas a sentir el bíceps estirarse más abajo.',muscleLabel:'Bíceps',ytQuery:'curl bayesiano en polea biceps'},
  {id:'e261',name:'Curl de Bíceps Predicador en Polea',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Curl sobre banco predicador con la polea baja: el brazo apoyado no puede hacer trampa y la tensión del cable no desaparece abajo. Abajo el codo queda en extensión completa con carga.',descSimple:'Apoya el brazo en el banco inclinado con la polea de abajo al frente. Sube doblando el codo y baja hasta estirar, pero sin soltar de golpe. El brazo no se despega del apoyo.',muscleLabel:'Bíceps',ytQuery:'curl predicador scott en polea'},
  {id:'e262',name:'Curl Martillo en Polea con Cuerda',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Curl con cuerda manteniendo el agarre neutro (palmas enfrentadas) de principio a fin: carga braquial y braquiorradial más que el bíceps, y es la variante que mejor tolera un codo sensible.',descSimple:'Con la cuerda en la polea de abajo, sube manteniendo las palmas mirándose todo el tiempo, como si llevaras dos martillos. Baja despacio. Trabaja el grosor del brazo.',muscleLabel:'Bíceps',ytQuery:'curl martillo en polea con cuerda'},
  {id:'e263',name:'Curl de Bíceps Predicador en Máquina',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Predicador guiado: el recorrido fijo permite llegar al fallo con seguridad porque no hay que estabilizar. Ideal como último ejercicio de brazo. Abajo el codo queda extendido bajo carga.',descSimple:'Siéntate y apoya la parte de atrás de los brazos en el cojín. Sube las manos doblando los codos y baja controlando hasta estirar. Como la máquina te guía, puedes exigirte más al final.',muscleLabel:'Bíceps',ytQuery:'curl de biceps predicador en maquina'},
  {id:'e264',name:'Curl de Bíceps Araña en Banco Inclinado',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['gym'],level:'I',desc:'Boca abajo sobre un banco inclinado con los brazos colgando en vertical: elimina por completo el balanceo del torso y el bíceps arranca desde su posición más estirada, con el codo en extensión bajo carga.',descSimple:'Acuéstate boca abajo en un banco inclinado con los brazos colgando y las mancuernas en las manos. Sube doblando solo los codos y baja despacio. Como estás acostado, no puedes hacer trampa con la espalda.',muscleLabel:'Bíceps',ytQuery:'curl araña spider curl banco inclinado'},
  {id:'e265',name:'Curl de Bíceps Arrastre con Barra',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'I',desc:'Drag curl: la barra sube ROZANDO el torso mientras los codos viajan hacia atrás, no hacia adelante. Quita al deltoides anterior de la ecuación y deja el trabajo en el bíceps.',descSimple:'Con la barra al frente, súbela pegadita al cuerpo mientras llevas los codos hacia atrás. La barra roza tu camiseta todo el camino. Se sube menos que en un curl normal y está bien.',muscleLabel:'Bíceps',ytQuery:'drag curl arrastre con barra biceps'},
  {id:'e266',name:'Curl de Bíceps con Barra Z',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'P',desc:'Curl con barra acodada: los quiebres dejan la muñeca en semisupinación y reducen la tensión sobre el antebrazo y el codo que produce la barra recta, con casi la misma carga sobre el bíceps.',descSimple:'Agarra la barra de las curvas, con las palmas hacia arriba. Sube doblando los codos sin moverlos del costado y baja despacio. Si la barra recta te molesta las muñecas, esta es tu opción.',muscleLabel:'Bíceps',ytQuery:'curl de biceps con barra z ez'},
  {id:'e267',name:'Curl Martillo en Banco Inclinado',muscle:'biceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'I',desc:'Martillo sentado en banco inclinado: el brazo queda por detrás de la línea del tronco, así que la cabeza larga arranca estirada. Agarre neutro de principio a fin.',descSimple:'Siéntate en un banco inclinado hacia atrás y deja los brazos colgando con las palmas mirándose. Sube sin girar las manos y baja despacio. Se siente más duro abajo, y eso es lo que buscamos.',muscleLabel:'Bíceps',ytQuery:'curl martillo en banco inclinado mancuernas'},
  {id:'e268',name:'Curl de Bíceps 21 con Barra',muscle:'biceps',type:'Aislamiento',sets:3,reps:21,icon:'🏋️',env:['gym'],level:'I',desc:'Serie de 21: siete repeticiones de la mitad de abajo, siete de la mitad de arriba y siete completas, sin soltar. Acumula tiempo bajo tensión con poco peso; se usa como remate, no como ejercicio principal.',descSimple:'Con poco peso: 7 repeticiones subiendo solo hasta la mitad, 7 desde la mitad hasta arriba, y 7 completas. Sin soltar la barra entre medio. Va a arder, es normal.',muscleLabel:'Bíceps',ytQuery:'curl 21 series biceps barra'},
  {id:'e269',name:'Extensión de Tríceps en Polea con Barra Recta',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Jalón de tríceps con barra recta en polea alta: agarre pronado fijo, el que más carga admite. Codos pegados al costado y quietos; solo se mueve el antebrazo.',descSimple:'De pie frente a la polea de arriba, agarra la barra con las palmas hacia abajo. Baja estirando los brazos sin mover los codos del costado y vuelve despacio.',muscleLabel:'Tríceps',ytQuery:'extension de triceps en polea con barra recta'},
  {id:'e270',name:'Extensión de Tríceps en Polea con Barra V',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Jalón de tríceps con la barra en V: el agarre angulado deja las muñecas en semipronación y suele tolerarse mejor que la barra recta cuando molesta el codo o la muñeca.',descSimple:'Igual que con la barra recta pero con la barra en forma de V, que deja las manos un poco giradas. Baja estirando los brazos con los codos quietos. Si la recta te incomoda, prueba esta.',muscleLabel:'Tríceps',ytQuery:'extension de triceps en polea con barra en v'},
  {id:'e271',name:'Extensión de Tríceps en Polea Agarre Supino a una Mano',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Jalón unilateral con la palma hacia arriba: el agarre supino carga preferentemente la cabeza medial y permite corregir diferencias entre brazos.',descSimple:'Con una sola mano en la polea de arriba y la palma mirando al techo, baja estirando el brazo sin mover el codo. Haz todas las repeticiones de un lado y cambia.',muscleLabel:'Tríceps',ytQuery:'extension de triceps polea agarre supino una mano'},
  {id:'e272',name:'Extensión de Tríceps Cruzada en Polea',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Jalón unilateral llevando la mano hacia la cadera CONTRARIA: el recorrido diagonal alarga el trayecto y acentúa la contracción final. Se hace con poco peso.',descSimple:'Con una mano en la polea de arriba, baja el brazo cruzando hacia la cadera del otro lado. El codo se queda quieto. Con poco peso: lo que se busca es el apretón del final.',muscleLabel:'Tríceps',ytQuery:'extension de triceps cruzada en polea'},
  {id:'e273',name:'Press Francés en Polea Tumbado',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Press francés acostado en banco con la polea baja detrás de la cabeza: la tensión no desaparece arriba y el vector del cable descarga algo la articulación respecto a la barra libre.',descSimple:'Acuéstate en un banco con la polea de abajo detrás de tu cabeza. Con la barra o la cuerda, estira los brazos hacia arriba doblando solo los codos. Vuelve despacio sin abrir los codos.',muscleLabel:'Tríceps',ytQuery:'press frances en polea tumbado triceps'},
  {id:'e274',name:'Extensión de Tríceps a una Mano con Mancuerna',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'P',desc:'Extensión sobre la cabeza con una mancuerna y un solo brazo: carga la cabeza larga en su posición más estirada. La mano libre puede sujetar el codo para que no se abra.',descSimple:'De pie o sentado, sube una mancuerna sobre tu cabeza y bájala por detrás doblando el codo. Sujeta ese codo con la otra mano para que no se abra. Sube estirando el brazo.',muscleLabel:'Tríceps',ytQuery:'extension de triceps a una mano con mancuerna'},
  {id:'e275',name:'Press JM con Barra',muscle:'triceps',type:'Compuesto',sets:3,reps:10,icon:'🏋️',env:['gym'],level:'A',desc:'Híbrido entre press cerrado y press francés: la barra baja hacia la parte alta del pecho con los codos adelantados. Permite cargar el tríceps más que cualquier aislamiento, y por eso mismo exige técnica.',descSimple:'Acostado en el banco con agarre cerrado, baja la barra hacia la parte alta del pecho dejando que los codos se vayan un poco adelante, y empuja. Es un ejercicio exigente: empieza con poco peso.',muscleLabel:'Tríceps',ytQuery:'press jm barra triceps tecnica'},
  {id:'e276',name:'Press Tate con Mancuernas',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'💪',env:['gym'],level:'I',desc:'Acostado, las mancuernas bajan girando hacia el pecho con los codos abiertos hasta que los discos casi se tocan. Trabaja la cabeza medial y la extensión terminal.',descSimple:'Acostado en el banco con dos mancuernas sobre el pecho, bájalas girando hacia adentro hasta que casi se toquen encima de ti, y empuja hacia arriba. Con poco peso.',muscleLabel:'Tríceps',ytQuery:'press tate mancuernas triceps'},
  {id:'e277',name:'Fondos Lastrados en Paralelas',muscle:'triceps',type:'Bodyweight',sets:3,reps:8,icon:'🏋️',env:['parque','gym'],level:'A',desc:'Fondos añadiendo peso con cinturón o chaleco, torso vertical para mantener el énfasis en tríceps. Progresión del fondo a peso corporal cuando ya se superan las 12 repeticiones limpias.',descSimple:'Fondos en las paralelas con un peso colgado del cinturón. Mantén el cuerpo lo más derecho posible para que trabaje el tríceps. Solo cuando ya hagas 12 fondos normales sin problema.',muscleLabel:'Tríceps',ytQuery:'fondos lastrados paralelas dips con peso'},
  {id:'e278',name:'Extensión de Tríceps en Máquina a una Mano',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Extensión guiada unilateral: el recorrido fijo permite llegar cerca del fallo sin estabilizar, y trabajar un brazo por vez destapa diferencias que la versión a dos manos esconde.',descSimple:'En la máquina de tríceps, usa un solo brazo por serie. Estira y vuelve despacio. Vas a notar si un brazo puede menos que el otro, que es justo lo que queremos ver.',muscleLabel:'Tríceps',ytQuery:'extension de triceps en maquina una mano'},
  {id:'e279',name:'Patada de Tríceps a Dos Manos en Polea',muscle:'triceps',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Patada con las dos manos en polea baja, tronco inclinado y codos fijos al costado. Como toda patada, el pico de carga cae con el tríceps ACORTADO y el codo nunca pasa por flexión profunda bajo carga.',descSimple:'Inclínate al frente frente a la polea de abajo, con los codos pegados al costado. Estira los dos brazos hacia atrás y vuelve despacio. Los codos no se mueven de su sitio.',muscleLabel:'Tríceps',ytQuery:'patada de triceps a dos manos en polea'},
  {id:'e280',name:'Extensión de Tríceps sobre la Cabeza con Barra Z',muscle:'triceps',type:'Aislamiento',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'I',desc:'Extensión sobre la cabeza con barra acodada, sentado con respaldo: los quiebres del agarre alivian la muñeca y el respaldo evita que la lumbar compense. Cabeza larga en máximo estiramiento.',descSimple:'Sentado con la espalda apoyada, sube la barra Z sobre tu cabeza y bájala por detrás doblando los codos. Los codos apuntan al frente y no se abren. Sube estirando.',muscleLabel:'Tríceps',ytQuery:'extension de triceps sobre la cabeza con barra z'},
  {id:'e281',name:'Aperturas Cruzadas en Polea Alta',muscle:'pecho',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'El aislamiento de pecho más usado del gimnasio: de pie entre dos poleas altas, los brazos bajan y se cruzan al frente con el codo casi fijo. La tensión no se pierde en ningún punto del recorrido.',descSimple:'Párate en medio de dos poleas altas con un agarre en cada mano. Baja los brazos juntándolos al frente hasta cruzarlos un poco, con los codos casi rectos. Vuelve despacio abriendo el pecho.',muscleLabel:'Pecho',ytQuery:'aperturas cruzadas en polea alta crossover'},
  {id:'e282',name:'Aperturas Cruzadas en Polea Baja',muscle:'pecho',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Cruce de abajo hacia arriba: el vector invertido carga la porción CLAVICULAR del pectoral, la misma que trabaja un press inclinado, pero en aislamiento.',descSimple:'Con las poleas abajo y un agarre en cada mano, sube los brazos juntándolos al frente a la altura de tu cara. Es el cruce al revés: se siente en la parte de arriba del pecho.',muscleLabel:'Pecho',ytQuery:'aperturas cruzadas en polea baja pecho superior'},
  {id:'e283',name:'Aperturas en Polea a una Mano',muscle:'pecho',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Cruce unilateral: permite un rango mayor porque el brazo puede cruzar la línea media sin chocar con el otro, y destapa diferencias entre lados.',descSimple:'Con una sola polea, cruza el brazo al frente hasta pasar la mitad de tu cuerpo. Al ser un brazo solo puedes llegar más lejos. Haz todas las repeticiones y cambia.',muscleLabel:'Pecho',ytQuery:'aperturas en polea a una mano unilateral'},
  {id:'e284',name:'Press Inclinado en Polea',muscle:'pecho',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Press en banco inclinado con las poleas bajas: tensión constante en todo el recorrido y menos carga articular que la barra en el punto de abajo.',descSimple:'Pon un banco inclinado entre dos poleas de abajo. Empuja los agarres hacia arriba y adelante como en un press, y baja despacio. La polea no te deja descansar arriba.',muscleLabel:'Pecho',ytQuery:'press inclinado en polea pecho'},
  {id:'e285',name:'Press de Pecho en Máquina Convergente',muscle:'pecho',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Máquina cuyos brazos se juntan al empujar, imitando el arco natural del pectoral. Recorrido guiado: permite llegar cerca del fallo sin estabilizar.',descSimple:'Siéntate y empuja los agarres al frente. Al estirar, las manos se van juntando solas. Como la máquina te guía, puedes exigirte más al final de la serie.',muscleLabel:'Pecho',ytQuery:'press de pecho en maquina convergente'},
  {id:'e286',name:'Press de Pecho en Máquina a una Mano',muscle:'pecho',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Press guiado unilateral: destapa la diferencia entre lados que la versión a dos manos esconde, y obliga al core a resistir la rotación.',descSimple:'En la máquina de press, empuja con un solo brazo por serie. Vas a notar si un lado puede menos, y de paso el abdomen trabaja para que no te gires.',muscleLabel:'Pecho',ytQuery:'press de pecho en maquina a una mano'},
  {id:'e287',name:'Aperturas en Máquina Inclinada',muscle:'pecho',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'I',desc:'Contractora con el respaldo inclinado: desplaza el énfasis de la apertura a la porción clavicular. Fin de rango bajo carga, como toda apertura.',descSimple:'En la máquina de aperturas con el asiento más inclinado, junta los brazos al frente y vuelve despacio. Se siente arriba del pecho, no en el medio.',muscleLabel:'Pecho',ytQuery:'aperturas en maquina inclinada pec deck'},
  {id:'e288',name:'Press de Pecho con Banda de Pie',muscle:'pecho',type:'Compuesto',sets:3,reps:15,icon:'🎗️',env:['casa','parque','gym'],level:'P',desc:'Press con la banda anclada detrás a la altura del pecho: la resistencia crece al final del empuje, justo donde el press con peso libre se vuelve fácil.',descSimple:'Ancla la banda detrás de ti a la altura del pecho y agarra un extremo en cada mano. Empuja al frente hasta estirar los brazos y vuelve despacio. Cuesta más al final.',muscleLabel:'Pecho',ytQuery:'press de pecho con banda elastica de pie'},
  {id:'e289',name:'Aperturas de Pecho con Banda',muscle:'pecho',type:'Aislamiento',sets:3,reps:15,icon:'🎗️',env:['casa','parque','gym'],level:'P',desc:'Apertura con banda anclada detrás: reproduce el cruce de poleas sin gimnasio. Fin de rango bajo carga.',descSimple:'Con la banda anclada detrás a la altura del pecho, abre los brazos y júntalos al frente con los codos casi rectos. Vuelve despacio sintiendo cómo se abre el pecho.',muscleLabel:'Pecho',ytQuery:'aperturas de pecho con banda elastica'},
  {id:'e290',name:'Press Inclinado en Multipower',muscle:'pecho',type:'Compuesto',sets:3,reps:10,icon:'🏗️',env:['gym'],level:'P',desc:'Press inclinado en barra guiada: el recorrido fijo permite cargar sin estabilizar y es la vía más segura para empujar fuerte sin compañero.',descSimple:'Pon un banco inclinado en la barra guiada. Baja la barra hasta la parte alta del pecho y empuja. Como la barra va en rieles, no tienes que equilibrarla.',muscleLabel:'Pecho',ytQuery:'press inclinado en multipower smith'},
  {id:'e291',name:'Press Declinado en Multipower',muscle:'pecho',type:'Compuesto',sets:3,reps:10,icon:'🏗️',env:['gym'],level:'P',desc:'Press declinado guiado: carga la porción esternal baja del pectoral con el hombro en una posición más cerrada que el press plano.',descSimple:'Con el banco declinado (con la cabeza más abajo) en la barra guiada, baja la barra al pecho bajo y empuja. Trabaja la parte de abajo del pecho.',muscleLabel:'Pecho',ytQuery:'press declinado en multipower smith'},
  {id:'e292',name:'Fondos en Paralelas para Pecho',muscle:'pecho',type:'Bodyweight',sets:3,reps:10,icon:'🤸',env:['parque','gym'],level:'A',desc:'Fondo con el torso INCLINADO al frente y los codos algo abiertos: esa inclinación pasa el trabajo del tríceps al pectoral bajo. Peso corporal a fin de rango.',descSimple:'En las paralelas, inclina el pecho hacia adelante y deja que los codos se abran un poco al bajar. Así trabaja el pecho y no el tríceps. Baja solo hasta donde no te moleste el hombro.',muscleLabel:'Pecho',ytQuery:'fondos en paralelas para pecho inclinado'},
  {id:'e293',name:'Flexiones Arqueras',muscle:'pecho',type:'Bodyweight',sets:3,reps:8,icon:'🏹',env:['corporal','casa','parque','gym'],level:'A',desc:'Lagartija cargando casi todo el peso en un brazo mientras el otro se estira de lado: progresión hacia la flexión a una mano sin equipo.',descSimple:'En posición de lagartija con las manos bien separadas, baja hacia un lado doblando ese brazo mientras el otro se queda estirado. Alterna lados. Es una lagartija difícil.',muscleLabel:'Pecho',ytQuery:'flexiones arqueras archer push up'},
  {id:'e294',name:'Flexiones con Palmada',muscle:'pecho',type:'HIIT',sets:3,reps:8,icon:'👏',env:['corporal','casa','parque','gym'],level:'A',desc:'Flexión pliométrica: se empuja con fuerza suficiente para despegar las manos. Trabaja potencia de empuje. Alto impacto sobre muñeca y codo.',descSimple:'Haz una lagartija empujando tan fuerte que las manos se despeguen del piso, y da una palmada antes de caer. Cae con los codos suaves. Solo si ya haces lagartijas normales sin problema.',muscleLabel:'Pecho',ytQuery:'flexiones con palmada pliometricas'},
  {id:'e295',name:'Press de Banca Agarre Amplio',muscle:'pecho',type:'Compuesto',sets:3,reps:8,icon:'🏋️',env:['gym'],level:'I',desc:'Press con las manos por fuera de la anchura estándar: acorta el recorrido y aumenta el estiramiento del pectoral, a costa de más tensión sobre el hombro.',descSimple:'Press de banca con las manos más separadas de lo normal. Baja menos y se siente más en el pecho. Si notas molestia en el hombro, vuelve al agarre de siempre.',muscleLabel:'Pecho',ytQuery:'press de banca agarre amplio ancho'},
  {id:'e296',name:'Pullover con Barra Z',muscle:'pecho',type:'Aislamiento',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'I',desc:'Pullover acostado con barra acodada: estira el pectoral y el dorsal por encima de la cabeza. Los quiebres del agarre alivian la muñeca frente a la barra recta.',descSimple:'Acostado en el banco, sujeta la barra Z sobre tu pecho y bájala por detrás de la cabeza con los codos casi rectos. Vuelve sintiendo cómo se estira todo el torso.',muscleLabel:'Pecho',ytQuery:'pullover con barra z acostado'},
  {id:'e297',name:'Jalón al Pecho Agarre Supino',muscle:'espalda',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Jalón con las palmas hacia ti: el hombro queda en rotación externa y el bíceps participa más, lo que suele permitir más carga y molestar menos el hombro.',descSimple:'En la polea de arriba, agarra la barra con las palmas mirándote. Jala hasta el pecho llevando los codos abajo y atrás. Suele sentirse más cómodo que el agarre normal.',muscleLabel:'Espalda',ytQuery:'jalon al pecho agarre supino'},
  {id:'e298',name:'Jalón a una Mano en Polea',muscle:'espalda',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Jalón unilateral con agarre simple: permite un recorrido mayor y que la escápula se mueva libre, además de igualar diferencias entre lados.',descSimple:'Con una sola mano en la polea de arriba, jala hacia abajo llevando el codo al costado. Deja que el hombro suba al estirar. Haz todas las repeticiones y cambia.',muscleLabel:'Espalda',ytQuery:'jalon al pecho a una mano en polea'},
  {id:'e299',name:'Jalón al Pecho con Cuerda',muscle:'espalda',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Jalón con cuerda y agarre neutro que se abre al final: la ausencia de barra deja que las manos sigan su trayectoria natural y suele aliviar hombro y codo.',descSimple:'Con la cuerda en la polea de arriba, jala hacia el pecho separando las manos al final. Al no tener barra, los brazos van por donde les es natural.',muscleLabel:'Espalda',ytQuery:'jalon al pecho con cuerda polea'},
  {id:'e300',name:'Remo Sentado en Polea con Barra Ancha',muscle:'espalda',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Remo sentado con barra ancha y codos abiertos: desplaza el trabajo del dorsal hacia el trapecio medio y el deltoides posterior, que es la zona que sostiene la postura.',descSimple:'Sentado en la polea baja, con una barra ancha, jala hacia el abdomen con los codos abiertos hacia los lados. Junta los omóplatos al final. Trabaja la parte alta de la espalda.',muscleLabel:'Espalda',ytQuery:'remo sentado en polea agarre amplio'},
  {id:'e301',name:'Pullover en Polea con Barra Recta',muscle:'espalda',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Pullover de pie en polea alta con los codos casi fijos: aísla el dorsal sin que el bíceps participe, porque el codo no se dobla.',descSimple:'De pie frente a la polea de arriba, con los brazos casi rectos baja la barra hasta los muslos en un arco. Los codos no se doblan: solo se mueve el hombro.',muscleLabel:'Espalda',ytQuery:'pullover en polea con barra recta dorsal'},
  {id:'e302',name:'Remo en Máquina a una Mano',muscle:'espalda',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'I',desc:'Remo guiado unilateral con apoyo pectoral: la lumbar queda descargada y se puede exigir un lado a la vez.',descSimple:'En la máquina de remo, jala con un solo brazo por serie. Como el pecho va apoyado, la espalda baja no trabaja: todo el esfuerzo va al dorsal.',muscleLabel:'Espalda',ytQuery:'remo en maquina a una mano apoyo pectoral'},
  {id:'e303',name:'Dominadas Agarre Neutro',muscle:'espalda',type:'Bodyweight',sets:3,reps:8,icon:'🤸',env:['parque','gym'],level:'A',desc:'Dominada con las palmas enfrentadas: el agarre neutro es el que menos tensión mete al hombro y a la muñeca de las tres variantes.',descSimple:'Cuélgate de dos agarres paralelos con las palmas mirándose y súbete hasta pasar la barbilla. Es la dominada que menos molesta hombros y muñecas.',muscleLabel:'Espalda',ytQuery:'dominadas agarre neutro paralelo'},
  {id:'e304',name:'Dominadas Lastradas',muscle:'espalda',type:'Bodyweight',sets:3,reps:6,icon:'🏋️',env:['parque','gym'],level:'A',desc:'Dominada con peso colgado del cinturón: la progresión cuando ya se superan las 12 repeticiones limpias a peso corporal.',descSimple:'Dominadas con un peso colgado del cinturón. Solo cuando ya hagas 12 dominadas normales seguidas y bien hechas. Sube menos repeticiones y con más peso.',muscleLabel:'Espalda',ytQuery:'dominadas lastradas con peso cinturon'},
  {id:'e305',name:'Remo Inclinado con Mancuernas a Dos Manos',muscle:'espalda',type:'Compuesto',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'I',desc:'Remo con el torso inclinado y las dos mancuernas a la vez: el agarre neutro permite llevar los codos pegados y la lumbar sostiene la posición todo el tiempo.',descSimple:'Inclina el torso al frente con la espalda recta y una mancuerna en cada mano. Jala las dos hacia las costillas y baja despacio. La espalda no se redondea en ningún momento.',muscleLabel:'Espalda',ytQuery:'remo inclinado con mancuernas a dos manos'},
  {id:'e306',name:'Remo en Punta con Barra',muscle:'espalda',type:'Compuesto',sets:3,reps:10,icon:'🏋️',env:['gym'],level:'I',desc:'Remo con un extremo de la barra anclado en el suelo (landmine): el arco del recorrido permite un rango mayor que el remo recto y carga el dorsal en toda su longitud.',descSimple:'Con una punta de la barra metida en una esquina o en el soporte, agarra el otro extremo e inclínate al frente. Jala hacia el abdomen y baja despacio, con la espalda recta.',muscleLabel:'Espalda',ytQuery:'remo en punta landmine row barra'},
  {id:'e307',name:'Remo Renegado con Mancuernas',muscle:'espalda',type:'Funcional',sets:3,reps:10,icon:'💪',env:['casa','gym'],level:'A',desc:'Remo desde posición de plancha alta sobre las mancuernas: la espalda trabaja mientras el core resiste la rotación. Peso corporal sobre la muñeca y el codo apoyados.',descSimple:'En posición de plancha con una mancuerna en cada mano, sube una hacia las costillas sin girar la cadera, bájala y cambia. Lo difícil es no torcerse.',muscleLabel:'Espalda',ytQuery:'remo renegado con mancuernas plancha'},
  {id:'e308',name:'Peso Muerto Sumo con Barra',muscle:'espalda',type:'Compuesto',sets:3,reps:6,icon:'🏋️',env:['gym'],level:'A',desc:'Peso muerto con base ancha y las manos por dentro de las piernas: el torso queda más vertical que en el convencional, así que la lumbar trabaja menos y la cadera más.',descSimple:'Con los pies bien abiertos y las manos por dentro de las rodillas, levanta la barra del piso empujando con las piernas. El pecho se queda más arriba que en el peso muerto normal.',muscleLabel:'Espalda',ytQuery:'peso muerto sumo con barra tecnica'},
  {id:'e309',name:'Peso Muerto Parcial en Rack',muscle:'espalda',type:'Compuesto',sets:3,reps:6,icon:'🏋️',env:['gym'],level:'A',desc:'Peso muerto que arranca por encima de la rodilla, con la barra apoyada en los soportes: elimina la parte baja del recorrido y permite cargar más en el bloqueo de cadera.',descSimple:'Pon la barra en los soportes a la altura de las rodillas y levántala desde ahí hasta ponerte derecho. Al recorrer menos, puedes usar más peso que en el peso muerto completo.',muscleLabel:'Espalda',ytQuery:'peso muerto parcial rack pull'},
  {id:'e310',name:'Encogimientos en Máquina',muscle:'espalda',type:'Aislamiento',sets:3,reps:15,icon:'🔩',env:['gym'],level:'P',desc:'Encogimiento guiado: sube y baja los hombros en vertical sin que la lumbar tenga que sostener el peso, a diferencia de la versión con barra.',descSimple:'En la máquina, sube los hombros hacia las orejas y bájalos despacio. Solo suben y bajan: no los gires. Como la máquina sostiene el peso, la espalda baja descansa.',muscleLabel:'Espalda',ytQuery:'encogimientos de trapecio en maquina'},
  {id:'e311',name:'Hiperextensiones en Máquina',muscle:'espalda',type:'Compuesto',sets:3,reps:15,icon:'🔩',env:['gym'],level:'I',desc:'Extensión de tronco guiada con respaldo y carga graduable: trabaja los erectores lumbares en un recorrido controlado.',descSimple:'En la máquina, empuja hacia atrás con la espalda hasta quedar derecho y vuelve despacio. Se trabaja la parte baja de la espalda, sin rebotes.',muscleLabel:'Espalda',ytQuery:'hiperextensiones lumbares en maquina'},
  {id:'e312',name:'Remo Invertido en Anillas',muscle:'espalda',type:'Bodyweight',sets:3,reps:12,icon:'🤸',env:['parque','gym'],level:'I',desc:'Remo horizontal con anillas o cintas: al ser colgantes, las manos rotan libres y el hombro elige su trayectoria. Se gradúa acercando o alejando los pies.',descSimple:'Cuélgate de las anillas con el cuerpo inclinado y los pies en el piso. Jala el pecho hacia las manos manteniéndote recto. Entre más horizontal te pongas, más cuesta.',muscleLabel:'Espalda',ytQuery:'remo invertido en anillas trx'},
  {id:'e313',name:'Remo al Mentón con Barra',muscle:'hombros',type:'Compuesto',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'I',desc:'Barra que sube pegada al cuerpo hasta el pecho alto con los codos por encima de las manos. Carga deltoides y trapecio. El agarre ANCHO y no subir más allá del pecho reducen el pinzamiento.',descSimple:'Con la barra al frente y las manos separadas, súbela pegada al cuerpo hasta la altura del pecho, con los codos apuntando arriba y afuera. No subas más. Si sientes pinchazo en el hombro, para.',muscleLabel:'Hombros',ytQuery:'remo al menton con barra upright row'},
  {id:'e314',name:'Remo al Mentón en Polea',muscle:'hombros',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Remo al mentón en polea baja con cuerda: la cuerda deja separar las manos al final, lo que reduce la rotación interna forzada frente a la barra.',descSimple:'Con la cuerda en la polea de abajo, súbela pegada al cuerpo hasta el pecho separando las manos al final. Los codos van arriba. Si molesta el hombro, no subas tanto.',muscleLabel:'Hombros',ytQuery:'remo al menton en polea con cuerda'},
  {id:'e315',name:'Elevación Lateral a una Mano en Polea',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Lateral en polea baja cruzando por detrás del cuerpo: la tensión existe desde el primer grado, al revés que con mancuerna, donde abajo no hay carga.',descSimple:'De pie de lado a la polea de abajo, con el cable cruzando por delante de ti, sube el brazo hacia el lado hasta la altura del hombro. Baja despacio.',muscleLabel:'Hombros',ytQuery:'elevacion lateral a una mano en polea'},
  {id:'e316',name:'Elevación Frontal en Polea',muscle:'hombros',type:'Aislamiento',sets:3,reps:12,icon:'🔗',env:['gym'],level:'P',desc:'Frontal desde la polea baja por detrás del cuerpo: mantiene tensión en el deltoides anterior en todo el arco.',descSimple:'De espaldas a la polea de abajo, sube el brazo al frente hasta la altura del hombro con el codo casi recto. Baja despacio.',muscleLabel:'Hombros',ytQuery:'elevacion frontal en polea deltoides anterior'},
  {id:'e317',name:'Elevación Posterior en Polea',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Cruce inverso con las poleas altas: aísla el deltoides posterior con tensión constante, la porción que casi siempre queda por detrás del resto del hombro.',descSimple:'Entre dos poleas altas, agarra el cable contrario con cada mano y ábrelos hacia afuera hasta quedar en cruz. Junta los omóplatos. Con poco peso.',muscleLabel:'Hombros',ytQuery:'elevacion posterior en polea reverse fly'},
  {id:'e318',name:'Elevación Frontal con Disco',muscle:'hombros',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Frontal sujetando un disco por los lados: el agarre neutro y simétrico reparte la carga entre los dos deltoides y es cómodo para la muñeca.',descSimple:'Agarra un disco por los bordes con las dos manos y súbelo al frente hasta la altura de los ojos, con los brazos casi rectos. Baja despacio.',muscleLabel:'Hombros',ytQuery:'elevacion frontal con disco'},
  {id:'e319',name:'Press Militar Sentado con Mancuernas',muscle:'hombros',type:'Compuesto',sets:3,reps:10,icon:'💪',env:['gym'],level:'I',desc:'Press vertical sentado con respaldo: el respaldo evita que la lumbar compense arqueándose y deja todo el trabajo en el deltoides.',descSimple:'Sentado con la espalda apoyada, sube las mancuernas desde la altura de los hombros hasta arriba y baja despacio. Al estar apoyado, no puedes arquear la espalda.',muscleLabel:'Hombros',ytQuery:'press militar sentado con mancuernas'},
  {id:'e320',name:'Press Militar en Multipower',muscle:'hombros',type:'Compuesto',sets:3,reps:10,icon:'🏗️',env:['gym'],level:'P',desc:'Press vertical en barra guiada: el riel elimina la estabilización y permite cargar el deltoides sin compañero.',descSimple:'Sentado bajo la barra guiada, empuja desde la altura de la barbilla hasta arriba y baja despacio. Como va en rieles, no tienes que equilibrarla.',muscleLabel:'Hombros',ytQuery:'press militar en multipower smith'},
  {id:'e321',name:'Elevación Lateral Inclinado con Mancuerna',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'💪',env:['casa','gym'],level:'I',desc:'Lateral sujetándose de algo e inclinando el cuerpo hacia el lado contrario: el brazo arranca cruzado, así que el deltoides medio trabaja desde el primer grado.',descSimple:'Agárrate de un soporte con una mano e inclina el cuerpo hacia ese lado. Con la otra mano sube la mancuerna hacia afuera. Al estar inclinado, el hombro trabaja desde abajo.',muscleLabel:'Hombros',ytQuery:'elevacion lateral inclinado leaning'},
  {id:'e322',name:'Pájaro en Banco Inclinado',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'💪',env:['casa','gym'],level:'P',desc:'Posterior boca abajo sobre un banco inclinado: el pecho apoyado impide balancear el torso, que es la trampa habitual del pájaro de pie.',descSimple:'Acuéstate boca abajo en un banco inclinado con una mancuerna en cada mano. Abre los brazos hacia los lados hasta la altura de los hombros. Con poco peso.',muscleLabel:'Hombros',ytQuery:'pajaro en banco inclinado deltoides posterior'},
  {id:'e323',name:'Press en Punta a una Mano',muscle:'hombros',type:'Compuesto',sets:3,reps:10,icon:'🏋️',env:['gym'],level:'I',desc:'Press con el extremo de la barra anclado (landmine): el recorrido es diagonal y no completamente vertical, así que el hombro trabaja en su plano natural.',descSimple:'Con una punta de la barra anclada, agarra el otro extremo a la altura del hombro y empuja arriba y adelante en diagonal. No llega a quedar sobre tu cabeza, y por eso suele ser cómodo.',muscleLabel:'Hombros',ytQuery:'press en punta landmine a una mano'},
  {id:'e324',name:'Rotación Externa en Polea',muscle:'hombros',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Rotación externa con el codo pegado al costado y la polea a la altura del ombligo: trabajo directo del manguito rotador con carga graduable.',descSimple:'De lado a la polea, con el codo pegado a las costillas doblado en ángulo recto, gira el antebrazo hacia afuera sin despegar el codo. Con muy poco peso.',muscleLabel:'Hombros',ytQuery:'rotacion externa en polea manguito rotador'},
  {id:'e325',name:'Press Cubano con Mancuernas',muscle:'hombros',type:'Compuesto',sets:3,reps:10,icon:'💪',env:['casa','gym'],level:'A',desc:'Tres movimientos encadenados: remo alto, rotación externa y press por encima de la cabeza. Trabaja el manguito bajo carga. Exige movilidad y se hace con muy poco peso.',descSimple:'Sube las mancuernas hasta la altura del pecho con los codos arriba, gira los antebrazos hacia arriba, y desde ahí empuja sobre tu cabeza. Deshaz el camino. Con muy poco peso.',muscleLabel:'Hombros',ytQuery:'press cubano con mancuernas manguito'},
  {id:'e326',name:'Press de Hombro en Máquina a una Mano',muscle:'hombros',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Press vertical guiado unilateral: destapa diferencias entre lados y obliga al core a resistir la inclinación lateral.',descSimple:'En la máquina de press de hombro, empuja con un solo brazo por serie. Vas a notar si un lado puede menos, y el abdomen trabaja para que no te inclines.',muscleLabel:'Hombros',ytQuery:'press de hombro en maquina a una mano'},
  {id:'e327',name:'Press de Pecho Isométrico Autorresistido',muscle:'pecho',type:'Isométrico',sets:3,reps:20,icon:'🙏',env:['corporal','casa','parque','gym'],level:'P',desc:'Pectoral sin equipo Y SIN APOYAR LA MANO: palmas enfrentadas a la altura del pecho, se empujan una contra otra y se sostiene la tensión. Es la única forma de cargar pecho con la muñeca en posición neutra, sin la extensión máxima que exige una lagartija.',descSimple:'Junta las palmas frente al pecho, como si rezaras, y empújalas una contra otra con fuerza. Aguanta unos segundos, suelta y repite. Trabajas el pecho sin apoyar las manos en el piso.',muscleLabel:'Pecho',ytQuery:'press isometrico de pecho palmas autorresistido'},
  {id:'e328',name:'Prensa de Pierna a una Pierna',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'I',desc:'Prensa unilateral: destapa la diferencia de fuerza entre lados que la versión a dos piernas esconde, y permite entrenar una pierna cuando la otra está limitada.',descSimple:'En la prensa, empuja con una sola pierna por serie, con el pie en el centro de la plataforma. Vas a notar si una puede menos que la otra.',muscleLabel:'Piernas',ytQuery:'prensa de pierna a una pierna unilateral'},
  {id:'e329',name:'Prensa de Pierna Pies Altos',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Pies en la parte alta de la plataforma: más flexión de cadera y menos de rodilla, así que el trabajo se desplaza a glúteo e isquios.',descSimple:'En la prensa, pon los pies en la parte de arriba de la plataforma. Vas a sentirlo más en el glúteo y la parte de atrás del muslo que en el cuádriceps.',muscleLabel:'Piernas',ytQuery:'prensa de pierna pies altos gluteo'},
  {id:'e330',name:'Prensa de Pierna Pies Bajos',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Pies en la parte baja de la plataforma: más flexión de rodilla, así que el cuádriceps se lleva casi todo el trabajo. Exige que el talón no se despegue.',descSimple:'En la prensa, pon los pies en la parte de abajo de la plataforma. Se siente todo en el muslo de adelante. Cuida que los talones no se levanten.',muscleLabel:'Piernas',ytQuery:'prensa de pierna pies bajos cuadriceps'},
  {id:'e331',name:'Extensión de Cuádriceps a una Pierna',muscle:'piernas',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Extensión unilateral: iguala diferencias entre lados y permite trabajar una pierna aislada. Carga directa sobre la rodilla en su recorrido final.',descSimple:'En la máquina de extensión, estira una sola pierna por serie. Sube sin dar tirón y baja despacio. Sirve para emparejar las dos piernas.',muscleLabel:'Piernas',ytQuery:'extension de cuadriceps a una pierna'},
  {id:'e332',name:'Curl Femoral a una Pierna',muscle:'piernas',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Curl femoral unilateral: el isquio es de los músculos donde más asimetría hay entre lados, y a dos piernas la fuerte compensa a la débil sin que se note.',descSimple:'En la máquina de curl femoral, dobla una sola pierna por serie. Baja despacio. Es donde más se nota si una pierna puede menos que la otra.',muscleLabel:'Piernas',ytQuery:'curl femoral a una pierna unilateral'},
  {id:'e333',name:'Curl Nórdico',muscle:'piernas',type:'Bodyweight',sets:3,reps:6,icon:'🦵',env:['casa','parque','gym'],level:'A',desc:'Excéntrico de isquios de rodillas con los tobillos fijos: el cuerpo baja recto resistiendo con la parte de atrás del muslo. Es de los ejercicios más exigentes que hay y se hace con muy pocas repeticiones.',descSimple:'De rodillas con alguien sujetándote los tobillos (o bajo algo firme), déjate caer al frente lo más lento que puedas manteniendo el cuerpo recto, y frena con las manos. Muy pocas repeticiones: al día siguiente se siente.',muscleLabel:'Piernas',ytQuery:'curl nordico isquiotibiales nordic ham curl'},
  {id:'e334',name:'Sentadilla Búlgara con Barra',muscle:'piernas',type:'Compuesto',sets:3,reps:10,icon:'🏋️',env:['gym'],level:'A',desc:'Búlgara cargando con barra al hombro en vez de mancuernas: permite mucha más carga, a costa de exigir equilibrio y movilidad de tobillo.',descSimple:'Con la barra en los hombros y el pie de atrás sobre un banco, baja doblando la pierna de adelante y sube. Cuesta mantener el equilibrio: empieza con la barra sola.',muscleLabel:'Piernas',ytQuery:'sentadilla bulgara con barra'},
  {id:'e335',name:'Sentadilla Zercher',muscle:'piernas',type:'Compuesto',sets:3,reps:8,icon:'🏋️',env:['gym'],level:'A',desc:'Sentadilla con la barra apoyada en el pliegue de los codos: obliga a un torso muy vertical y exige mucho de la zona lumbar y del core para no irse adelante. Incómoda por definición.',descSimple:'Sostén la barra en el hueco de los codos, pegada al cuerpo, y haz sentadilla. El pecho se queda muy derecho. Es incómoda en los brazos: usa una toalla si molesta.',muscleLabel:'Piernas',ytQuery:'sentadilla zercher tecnica'},
  {id:'e336',name:'Sentadilla Cosaco',muscle:'piernas',type:'Compuesto',sets:3,reps:10,icon:'🦵',env:['corporal','casa','parque','gym'],level:'I',desc:'Sentadilla lateral profunda: se baja sobre una pierna mientras la otra queda estirada de lado. Lleva el aductor de la pierna estirada a su máximo estiramiento.',descSimple:'De pie con los pies bien separados, baja sobre una pierna doblándola mientras la otra se queda estirada a un lado. Alterna. Vas a sentir el estirón por dentro del muslo.',muscleLabel:'Piernas',ytQuery:'sentadilla cosaco cossack squat'},
  {id:'e337',name:'Sentadilla Sissy',muscle:'piernas',type:'Aislamiento',sets:3,reps:12,icon:'🦵',env:['casa','parque','gym'],level:'A',desc:'Sentadilla llevando las rodillas muy adelante y el cuerpo hacia atrás en línea recta: aísla el cuádriceps en flexión máxima de rodilla. Muy exigente para la rodilla, por eso va a peso corporal y con apoyo.',descSimple:'Agarrado de un soporte, deja que las rodillas se vayan adelante mientras te inclinas atrás con el cuerpo recto, subiendo los talones. Baja hasta donde NO te moleste la rodilla.',muscleLabel:'Piernas',ytQuery:'sentadilla sissy squat cuadriceps'},
  {id:'e338',name:'Peso Muerto con Barra Hexagonal',muscle:'piernas',type:'Compuesto',sets:3,reps:8,icon:'🏋️',env:['gym'],level:'I',desc:'Peso muerto dentro de una barra hexagonal, con las manos a los lados del cuerpo: la carga queda alineada con el centro de gravedad, así que la lumbar trabaja menos que en el convencional y la rodilla algo más.',descSimple:'Métete dentro de la barra hexagonal, agarra los mangos de los lados y levanta empujando el piso con las piernas. Es la versión más fácil de aprender del peso muerto.',muscleLabel:'Piernas',ytQuery:'peso muerto con barra hexagonal trap bar'},
  {id:'e339',name:'Peso Muerto Rumano en Polea',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Bisagra de cadera con la polea baja: la tensión constante mantiene cargado el isquio también arriba, y el vector horizontal del cable obliga a llevar la cadera atrás de verdad.',descSimple:'De pie frente a la polea de abajo con la barra en las manos, lleva la cadera hacia atrás bajando la barra por delante de las piernas, con la espalda recta. Sube apretando el glúteo.',muscleLabel:'Piernas',ytQuery:'peso muerto rumano en polea'},
  {id:'e340',name:'Aducción de Cadera en Polea de Pie',muscle:'piernas',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Aducción unilateral de pie con tobillera: la pierna cruza hacia la línea media contra la polea. Trabaja el aductor en cadena abierta y con tensión constante.',descSimple:'Con la tobillera en la polea de abajo y de lado a la máquina, lleva la pierna de afuera hacia adentro cruzando por delante de la otra. Vuelve despacio.',muscleLabel:'Piernas',ytQuery:'aduccion de cadera en polea de pie'},
  {id:'e341',name:'Elevación de Talones a una Pierna',muscle:'piernas',type:'Aislamiento',sets:3,reps:15,icon:'🦵',env:['corporal','casa','parque','gym'],level:'P',desc:'Gemelo unilateral: el gemelo aguanta el peso del cuerpo entero con facilidad, así que a dos piernas casi nunca se carga lo suficiente. A una pierna sí.',descSimple:'Párate en un escalón sobre una sola pierna, agarrado de algo. Sube el talón lo más alto que puedas y baja despacio hasta sentir el estirón. Cambia de pierna.',muscleLabel:'Piernas',ytQuery:'elevacion de talones a una pierna gemelo'},
  {id:'e342',name:'Elevación de Talones en Multipower',muscle:'piernas',type:'Aislamiento',sets:3,reps:15,icon:'🏗️',env:['gym'],level:'P',desc:'Gemelo de pie con la barra guiada al hombro: permite cargar mucho sin problemas de equilibrio, que es lo que limita la versión con mancuernas.',descSimple:'Con la barra guiada en los hombros y la punta de los pies sobre un disco o escalón, sube los talones lo más alto que puedas y baja despacio.',muscleLabel:'Piernas',ytQuery:'elevacion de talones en multipower smith'},
  {id:'e343',name:'Bajada Controlada de Escalón',muscle:'piernas',type:'Bodyweight',sets:3,reps:10,icon:'🦵',env:['corporal','casa','parque','gym'],level:'P',desc:'Se sube al escalón y se BAJA muy despacio con una sola pierna: el trabajo es excéntrico y controla la rodilla en el rango donde suele fallar. Es la progresión previa a la sentadilla a una pierna.',descSimple:'Párate sobre un escalón con una pierna. Baja el otro pie hasta rozar el piso lo más lento que puedas, y vuelve a subir. La pierna de arriba hace todo el trabajo.',muscleLabel:'Piernas',ytQuery:'bajada controlada de escalon step down'},
  {id:'e344',name:'Zancada en Polea',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔗',env:['gym'],level:'I',desc:'Zancada sosteniendo la polea baja: el cable tira hacia atrás, así que obliga a mantener el torso erguido y añade tensión constante que la mancuerna no da.',descSimple:'De frente a la polea de abajo con el agarre en las manos, da un paso atrás y baja en zancada. El cable te jala hacia adelante: mantente derecho.',muscleLabel:'Piernas',ytQuery:'zancada en polea desplante cable'},
  {id:'e345',name:'Prensa Vertical',muscle:'piernas',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'I',desc:'Prensa con la carga directamente encima: acostado boca arriba se empuja el peso en vertical. Recorrido corto y mucha tensión en la parte profunda del movimiento.',descSimple:'Acostado bajo la máquina, empuja la plataforma hacia arriba con los pies. Baja hasta donde la espalda baja no se despegue del respaldo.',muscleLabel:'Piernas',ytQuery:'prensa vertical de pierna'},
  {id:'e346',name:'Extensión de Cadera en Máquina',muscle:'piernas',type:'Aislamiento',sets:3,reps:12,icon:'🔩',env:['gym'],level:'P',desc:'Extensión de cadera guiada de pie: aísla el glúteo mayor con recorrido fijo y sin que la lumbar tenga que estabilizar, a diferencia del hip thrust.',descSimple:'De pie en la máquina, empuja la pierna hacia atrás desde la cadera y vuelve despacio. La espalda se queda quieta: solo se mueve la pierna.',muscleLabel:'Piernas',ytQuery:'extension de cadera en maquina gluteo'},
  {id:'e347',name:'Sentadilla con Chaleco Lastrado',muscle:'piernas',type:'Bodyweight',sets:3,reps:15,icon:'🦺',env:['casa','parque','gym'],level:'I',desc:'Sentadilla a peso corporal con chaleco o mochila cargada: la progresión natural cuando la sentadilla sin peso ya no cuesta y no hay barra disponible.',descSimple:'Sentadilla normal pero con un chaleco con peso o una mochila cargada en la espalda. Es el siguiente paso cuando las sentadillas sin peso ya te salen fáciles.',muscleLabel:'Piernas',ytQuery:'sentadilla con chaleco lastrado mochila'},
  {id:'e348',name:'Hip Thrust a una Pierna en Máquina',muscle:'gluteo',type:'Compuesto',sets:3,reps:12,icon:'🔩',env:['gym'],level:'I',desc:'Hip thrust unilateral guiado: la máquina estabiliza y permite cargar un lado a la vez, que es donde aparece la asimetría de glúteo que la versión a dos piernas esconde.',descSimple:'En la máquina de hip thrust, empuja con una sola pierna por serie. Aprieta el glúteo arriba y baja despacio. Vas a notar si un lado puede menos.',muscleLabel:'Glúteo',ytQuery:'hip thrust a una pierna en maquina'},
  {id:'e349',name:'Hip Thrust con Banda',muscle:'gluteo',type:'Compuesto',sets:3,reps:15,icon:'🎗️',env:['casa','parque','gym'],level:'P',desc:'Hip thrust con banda sobre la cadera: la resistencia crece justo arriba, que es donde el glúteo termina de contraerse y donde la banda pesa más.',descSimple:'Con la espalda apoyada en un sofá o banco y la banda pasando por encima de la cadera, sube la cadera apretando el glúteo. Arriba es donde más cuesta.',muscleLabel:'Glúteo',ytQuery:'hip thrust con banda elastica'},
  {id:'e350',name:'Puente de Glúteo con Barra',muscle:'gluteo',type:'Compuesto',sets:3,reps:12,icon:'🏋️',env:['gym'],level:'P',desc:'Puente en el suelo con la barra sobre la cadera: recorrido más corto que el hip thrust en banco, pero se aprende antes y no necesita montaje.',descSimple:'Acostado en el piso con las rodillas dobladas y la barra sobre la cadera, sube la cadera hasta hacer una línea recta y aprieta el glúteo. Baja despacio.',muscleLabel:'Glúteo',ytQuery:'puente de gluteo con barra piso'},
  {id:'e351',name:'Abducción de Cadera en Polea de Pie',muscle:'gluteo',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Abducción unilateral de pie con tobillera: la polea da tensión constante y la posición de pie obliga a la pierna de apoyo a estabilizar la pelvis, que es trabajo extra de glúteo medio.',descSimple:'Con la tobillera en la polea de abajo, agárrate de la máquina y lleva la pierna de afuera hacia el lado sin inclinar el cuerpo. Vuelve despacio.',muscleLabel:'Glúteo',ytQuery:'abduccion de cadera en polea de pie gluteo medio'},
  {id:'e352',name:'Abducción de Cadera en Máquina con Torso Inclinado',muscle:'gluteo',type:'Aislamiento',sets:3,reps:15,icon:'🔩',env:['gym'],level:'I',desc:'Misma máquina de abducción pero inclinando el torso al frente: esa inclinación cambia la línea de tracción y sesga el trabajo hacia la porción superior del glúteo mayor.',descSimple:'En la máquina de abducción, en vez de quedarte derecho, inclina el pecho hacia adelante y abre las piernas desde ahí. Se siente más arriba en el glúteo.',muscleLabel:'Glúteo',ytQuery:'abduccion de cadera en maquina torso inclinado'},
  {id:'e353',name:'Patada de Glúteo en Multipower',muscle:'gluteo',type:'Aislamiento',sets:3,reps:12,icon:'🏗️',env:['gym'],level:'I',desc:'Extensión de cadera con la barra guiada apoyada en la parte de atrás del muslo: permite cargar la patada mucho más que la polea, con el recorrido controlado por el riel.',descSimple:'Apoyado de frente a la barra guiada, pon la parte de atrás del muslo bajo la barra y empuja hacia arriba con el talón. La espalda no se arquea.',muscleLabel:'Glúteo',ytQuery:'patada de gluteo en multipower smith'},
  {id:'e354',name:'Peso Muerto Sumo con Mancuerna',muscle:'gluteo',type:'Compuesto',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'P',desc:'Bisagra con base ancha y una mancuerna entre las piernas: el torso queda más vertical que en el rumano y el trabajo se reparte entre glúteo y aductor.',descSimple:'Con los pies bien abiertos y las puntas hacia afuera, sostén una mancuerna entre las piernas y baja llevando la cadera atrás con la espalda recta. Sube apretando el glúteo.',muscleLabel:'Glúteo',ytQuery:'peso muerto sumo con mancuerna'},
  {id:'e355',name:'Zancada Lateral con Mancuernas',muscle:'gluteo',type:'Compuesto',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'I',desc:'Paso lateral amplio cargando: se baja sobre la pierna que da el paso mientras la otra se estira. Trabaja glúteo medio de la que carga y estira el aductor de la otra.',descSimple:'Con una mancuerna en cada mano, da un paso grande hacia el lado y baja sobre esa pierna, dejando la otra estirada. Empuja para volver al centro y cambia de lado.',muscleLabel:'Glúteo',ytQuery:'zancada lateral con mancuernas'},
  {id:'e356',name:'Step-up Lateral con Mancuernas',muscle:'gluteo',type:'Funcional',sets:3,reps:12,icon:'💪',env:['casa','gym'],level:'I',desc:'Subida a un cajón desde el lado: la pierna de arriba trabaja en abducción y extensión a la vez, que es la función del glúteo medio con carga.',descSimple:'Párate de lado a un cajón o banco. Sube apoyando el pie de ese lado, sin impulsarte con el otro. Baja despacio y repite antes de cambiar.',muscleLabel:'Glúteo',ytQuery:'step up lateral con mancuernas cajon'},
  {id:'e357',name:'Hiperextensión de Cadera en Banco Romano',muscle:'gluteo',type:'Compuesto',sets:3,reps:15,icon:'🔩',env:['gym'],level:'P',desc:'Extensión en el banco a 45° con la espalda REDONDEADA a propósito y el movimiento saliendo de la cadera: así el trabajo va al glúteo y no a los erectores. Se hace sin peso o con muy poco.',descSimple:'En el banco inclinado, baja el tronco y sube empujando desde la cadera, apretando el glúteo arriba. No arquees la espalda al final: te quedas en línea recta.',muscleLabel:'Glúteo',ytQuery:'hiperextension de cadera banco romano gluteo'},
  {id:'e358',name:'Patada de Glúteo en Polea Alta',muscle:'gluteo',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'P',desc:'Patada con la polea ARRIBA: el cable tira hacia adelante y arriba, así que la resistencia es máxima al final de la extensión, al revés que con la polea baja.',descSimple:'Con la tobillera en la polea de arriba, inclínate al frente agarrado de la máquina y lleva la pierna hacia atrás. Se siente el apretón al final del movimiento.',muscleLabel:'Glúteo',ytQuery:'patada de gluteo en polea alta'},
  {id:'e359',name:'Puente de Glúteo con Abducción en Banda',muscle:'gluteo',type:'Bodyweight',sets:3,reps:20,icon:'🎗️',env:['corporal','casa','parque','gym'],level:'P',desc:'Puente empujando las rodillas contra una banda: añade abducción a la extensión de cadera, así que glúteo mayor y medio trabajan a la vez. Cero equipo pesado.',descSimple:'Acostado con las rodillas dobladas y una banda alrededor de ellas, sube la cadera mientras separas las rodillas contra la banda. Aprieta arriba y baja despacio.',muscleLabel:'Glúteo',ytQuery:'puente de gluteo con banda en rodillas'},
  {id:'e360',name:'Leñador en Polea de Alto a Bajo',muscle:'core',type:'Compuesto',sets:3,reps:12,icon:'🪓',env:['gym'],level:'I',desc:'Rotación de tronco con la polea ARRIBA, llevando el agarre en diagonal hacia la cadera contraria. El giro sale del tronco con la cadera estable; es trabajo de oblicuos bajo carga rotacional.',descSimple:'Con la polea de arriba, de lado a la máquina, jala el agarre en diagonal hacia la cadera del lado contrario girando el tronco. Los pies no se mueven. Vuelve despacio.',muscleLabel:'Core',ytQuery:'leñador en polea de alto a bajo wood chop'},
  {id:'e361',name:'Leñador en Polea de Bajo a Alto',muscle:'core',type:'Compuesto',sets:3,reps:12,icon:'🪓',env:['gym'],level:'I',desc:'Rotación ascendente con la polea abajo: el patrón contrario al leñador clásico, y el que más participación de cadera y glúteo mete al giro.',descSimple:'Con la polea de abajo, de lado a la máquina, sube el agarre en diagonal hacia el hombro contrario girando el tronco. Acompaña con la cadera. Vuelve despacio.',muscleLabel:'Core',ytQuery:'leñador en polea de bajo a alto'},
  {id:'e362',name:'Giro Ruso (Russian Twist) en Polea',muscle:'core',type:'Aislamiento',sets:3,reps:15,icon:'🔗',env:['gym'],level:'I',desc:'Giro de tronco horizontal contra la polea, con los brazos estirados al frente y la cadera fija. Rotación de columna bajo carga: se hace con poco peso y recorrido corto.',descSimple:'De lado a la polea a la altura del pecho, con los brazos estirados al frente, gira el tronco alejándote de la máquina. La cadera se queda quieta. Con poco peso.',muscleLabel:'Core',ytQuery:'giro ruso russian twist en polea'},
  {id:'e363',name:'Elevación de Piernas en Silla Romana',muscle:'core',type:'Bodyweight',sets:3,reps:12,icon:'🦵',env:['gym'],level:'I',desc:'Elevación de piernas apoyado en los antebrazos: la silla romana quita el balanceo del colgado y deja el trabajo en el abdomen bajo. Flexión de cadera y columna contra gravedad.',descSimple:'Apoya los antebrazos en la silla romana con la espalda pegada al respaldo. Sube las rodillas hacia el pecho sin balancearte y baja despacio.',muscleLabel:'Core',ytQuery:'elevacion de piernas en silla romana'},
  {id:'e364',name:'Elevación de Rodillas Colgado',muscle:'core',type:'Bodyweight',sets:3,reps:12,icon:'🤸',env:['parque','gym'],level:'I',desc:'Colgado de la barra, se suben las rodillas al pecho enrollando la pelvis al final. La versión con rodillas dobladas es la progresión previa a subir las piernas estiradas. Exige agarre sostenido.',descSimple:'Cuélgate de la barra y sube las rodillas hacia el pecho, enrollando la cadera al final. Sin balancearte. Si el agarre te falla antes que el abdomen, usa unas correas.',muscleLabel:'Core',ytQuery:'elevacion de rodillas colgado en barra'},
  {id:'e365',name:'Plancha con Disco en la Espalda',muscle:'core',type:'Isométrico',sets:3,reps:30,icon:'🔩',env:['gym'],level:'I',desc:'Plancha de antebrazos con un disco sobre la zona lumbar: la progresión de la plancha cuando aguantar dos minutos ya no cuesta. Añade carga sin cambiar el patrón.',descSimple:'Plancha normal sobre los antebrazos con un disco puesto en la espalda baja (que alguien te lo ponga). Aguanta con el cuerpo recto. Es la plancha de siempre pero con peso.',muscleLabel:'Core',ytQuery:'plancha con disco en la espalda lastrada'},
  {id:'e366',name:'Crunch Completo con Disco (Sit-up)',muscle:'core',type:'Aislamiento',sets:3,reps:15,icon:'🔩',env:['casa','gym'],level:'I',desc:'Abdominal completo subiendo hasta sentarse, con un disco contra el pecho. Flexión de columna bajo carga: se acumula mucha compresión, por eso va con poco peso y sin tirones.',descSimple:'Acostado con las rodillas dobladas y un disco contra el pecho, sube hasta quedar sentado y baja despacio. Sin dar tirón con el cuello ni con los brazos.',muscleLabel:'Core',ytQuery:'crunch completo sit up con disco'},
  {id:'e367',name:'Dragon Flag',muscle:'core',type:'Bodyweight',sets:3,reps:6,icon:'🐉',env:['parque','gym'],level:'A',desc:'Acostado agarrado detrás de la cabeza, el cuerpo entero sube y baja RECTO como una tabla desde los hombros. Es de los ejercicios de abdomen más exigentes que existen y carga muchísimo la columna.',descSimple:'Acostado en un banco, agárrate por detrás de la cabeza y sube todo el cuerpo recto como una tabla, apoyado solo en los hombros. Baja lo más lento que puedas sin doblarte. Muy difícil.',muscleLabel:'Core',ytQuery:'dragon flag tecnica progresion'},
  {id:'e368',name:'Hollow Rock',muscle:'core',type:'Bodyweight',sets:3,reps:15,icon:'🌙',env:['corporal','casa','parque','gym'],level:'I',desc:'Desde la posición hollow (lumbar pegada al piso, brazos y piernas estirados), el cuerpo se mece como una mecedora sin perder la forma. Trabaja el abdomen en isometría dinámica.',descSimple:'Acostado con la espalda baja bien pegada al piso, estira brazos y piernas y mécete hacia adelante y atrás como una mecedora, sin que la espalda se despegue.',muscleLabel:'Core',ytQuery:'hollow rock abdomen gimnastico'},
  {id:'e369',name:'Crunch Bicicleta',muscle:'core',type:'Bodyweight',sets:3,reps:20,icon:'🚲',env:['corporal','casa','parque','gym'],level:'P',desc:'Crunch alternando codo con rodilla contraria mientras la otra pierna se estira: suma rotación a la flexión, así que oblicuos y recto trabajan juntos. Se hace despacio, no rápido.',descSimple:'Acostado con las manos en la nuca, lleva un codo hacia la rodilla contraria mientras estiras la otra pierna. Alterna despacio, como pedaleando. No jales del cuello.',muscleLabel:'Core',ytQuery:'crunch bicicleta abdominales oblicuos'},
  {id:'e370',name:'Crunch en V (V-Ups)',muscle:'core',type:'Bodyweight',sets:3,reps:12,icon:'📐',env:['corporal','casa','parque','gym'],level:'I',desc:'Brazos y piernas suben a la vez hasta tocarse formando una V, y bajan sin apoyarse. Flexión de columna y de cadera al tiempo, con el abdomen sosteniendo todo el recorrido.',descSimple:'Acostado boca arriba con brazos y piernas estirados, súbelos a la vez hasta tocarte los pies formando una V. Baja sin dejar caer del todo.',muscleLabel:'Core',ytQuery:'crunch en v v ups abdominales'},
  {id:'e371',name:'Plancha Lateral con Elevación de Cadera',muscle:'core',type:'Bodyweight',sets:3,reps:12,icon:'📐',env:['corporal','casa','parque','gym'],level:'I',desc:'Plancha lateral bajando y subiendo la cadera: convierte una isometría en trabajo dinámico del oblicuo y del cuadrado lumbar, que es lo que estabiliza el tronco al caminar.',descSimple:'En plancha lateral apoyado en el antebrazo, baja la cadera hasta casi tocar el piso y súbela otra vez. Repite y cambia de lado. El cuerpo se queda en línea.',muscleLabel:'Core',ytQuery:'plancha lateral con elevacion de cadera'},
  {id:'e372',name:'Ski Erg',muscle:'cardio',type:'Cardio',sets:1,reps:15,icon:'🎿',env:['gym'],level:'P',desc:'Máquina que imita el impulso de esquí de fondo: se tira de dos agarres desde arriba hacia la cadera usando tronco y cadera. Cardio de tren superior con cero impacto.',descSimple:'Agarra los dos mangos por encima de la cabeza y jálalos hacia abajo doblando la cadera, como si empujaras con bastones de esquí. Trabaja brazos y tronco sin correr.',muscleLabel:'Cardio',ytQuery:'ski erg maquina tecnica'},
  {id:'e373',name:'Bicicleta de Asalto',muscle:'cardio',type:'Cardio',sets:1,reps:15,icon:'🌀',env:['gym'],level:'P',desc:'Bicicleta de ventilador con manubrios móviles: la resistencia sube con el esfuerzo, así que se auto-regula. Trabaja piernas y brazos a la vez y es de lo más duro que hay en intervalos.',descSimple:'Bicicleta con aspa y manubrios que se mueven. Pedalea empujando y jalando los manubrios a la vez. Entre más fuerte le des, más se resiste: perfecta para intervalos.',muscleLabel:'Cardio',ytQuery:'bicicleta de asalto assault bike'},
  {id:'e374',name:'Escalador Vertical',muscle:'cardio',type:'Cardio',sets:1,reps:12,icon:'🧗',env:['gym'],level:'I',desc:'Máquina de escalada vertical con brazos y piernas: patrón cruzado de cuerpo completo y cero impacto sobre la articulación. Sube la frecuencia cardíaca muy rápido.',descSimple:'En la máquina de escalar, mueve brazo y pierna contrarios al tiempo, como trepando. Sube pulsaciones rapidísimo y no golpea las rodillas.',muscleLabel:'Cardio',ytQuery:'escalador vertical maquina versaclimber'},
  {id:'e375',name:'Caminata en Cinta Inclinada',muscle:'cardio',type:'Cardio',sets:1,reps:30,icon:'⛰️',env:['gym'],level:'P',desc:'Caminata a buena inclinación y ritmo moderado: quema mucho con impacto casi nulo, y es la opción cuando correr no es viable por rodilla, tobillo o peso corporal.',descSimple:'Camina en la cinta con buena inclinación, a un ritmo en el que puedas hablar con dificultad. Sin agarrarte de los pasamanos. Quema mucho y no golpea las rodillas.',muscleLabel:'Cardio',ytQuery:'caminata en cinta inclinada incline walk'},
  {id:'e376',name:'Empuje de Trineo',muscle:'cardio',type:'Funcional',sets:4,reps:20,icon:'🛷',env:['gym'],level:'I',desc:'Se empuja el trineo cargado con los brazos extendidos y el cuerpo inclinado. Cardio y fuerza a la vez, y como no tiene fase excéntrica casi no deja agujetas.',descSimple:'Con el trineo cargado, agárrate de los postes con los brazos estirados, inclínate al frente y empújalo caminando fuerte. Se siente en las piernas y en el pecho.',muscleLabel:'Cardio',ytQuery:'empuje de trineo sled push'},
  {id:'e377',name:'Arrastre de Trineo',muscle:'cardio',type:'Funcional',sets:4,reps:20,icon:'🪢',env:['gym'],level:'I',desc:'Se camina hacia atrás jalando el trineo con una cuerda o arnés: el trabajo cae sobre el cuádriceps y la rodilla trabaja sin impacto, por eso se usa mucho en readaptación.',descSimple:'Con la cuerda del trineo en las manos, camina hacia atrás jalándolo. Los muslos de adelante van a arder. Es de las formas más amables de trabajar la rodilla.',muscleLabel:'Cardio',ytQuery:'arrastre de trineo sled drag hacia atras'},
  {id:'e378',name:'Comba con Doble Salto',muscle:'cardio',type:'HIIT',sets:4,reps:20,icon:'🪁',env:['corporal','casa','parque','gym'],level:'A',desc:'Salto de cuerda pasando la comba DOS veces por salto: exige mucha más altura y velocidad de muñeca. Alto impacto y muy demandante.',descSimple:'Salto de cuerda pasando la cuerda dos veces en cada salto. Hay que saltar más alto y girar la muñeca más rápido. Solo cuando ya saltes cuerda normal sin enredarte.',muscleLabel:'Cardio',ytQuery:'comba doble salto double under'},
  {id:'e379',name:'Burpee con Salto al Cajón',muscle:'cardio',type:'HIIT',sets:4,reps:10,icon:'📦',env:['parque','gym'],level:'A',desc:'Burpee completo que termina en salto sobre un cajón en vez de en el aire: suma altura y aterrizaje controlado. Muy alto impacto.',descSimple:'Haz un burpee y al levantarte salta arriba de un cajón. Baja del cajón caminando, no saltando. Es de los ejercicios más duros que hay.',muscleLabel:'Cardio',ytQuery:'burpee con salto al cajon box jump'},
