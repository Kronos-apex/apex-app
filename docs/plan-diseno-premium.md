...
- [~] `#p-detail` (el más denso) — **PASE 1 HECHO avi-v339**: la tarjeta de gestión de rutina (`renderDetailRoutines`), foco en el área #4 diferida. (1) 🟡 táctil #4: los 3 micro-botones (plantilla/editar/borrar) medían ~20px → `min-height:36px` centrado, sin ensanchar la fila 360px (NO `.hit40`: son adyacentes, sus overlays chocarían) + `aria-label`; (2) 🟡 cohesión #5: 📂 (único emoji crudo de los 3) → nuevo ícono `folder` (calca el de Plantillas del menú) vía `_coIco`; `⏱`→timer, `🔗`→link en meta/filas (detalle + preview de "Generar semana"), igualando la tarjeta del asesorado. DEFERIDO (pase 2): tiers ⭐/👑 (header tag + selector de nivel), 💳 pago, 👇 chat vacío, 📊 valoración, emojis de calentamiento
