// Verificación E2E de la tarjeta HÁBITOS DE HOY — 💧 agua por vasos (v300, pedido
// Camilo 2026-07-09). Login real con la CUENTA QA (qa-harness). El sello v298
// (cloudWriteSealed) corta toda escritura a la nube en localhost, así que los taps
// de prueba jamás tocan producción. La tarjeta vive FUERA de cn-today-body → sale
// con entreno, en día de descanso y sin rutinas.
...
try {
  ...
  let s = JSON.parse(await ev(`JSON.stringify((()=>{const c=DB.clients.find(x=>x.id===CUR.clientId);
    return {card:!!document.querySelector('#cn-habits .hb-card'),sub:(document.querySelector('#cn-habits .hb-sub')||{}).textContent,
      goal:_waterGoalFor(c),n:waterToday(c.habits),dots:document.querySelectorAll('#cn-habits .hb-dot').length};})())`));
