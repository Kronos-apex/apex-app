...
const LEGAL_V='2026-07-20-borrador'; // 2026-07-20: +sección 9 «Comunidad» en la política (C4)
const LEGAL_DOCS={
...
function renderGenPreview(c,res){
  ...
  const cards=res.routines.map(r=>{
    const totS=r.exercises.reduce((s,e)=>s+(parseInt(e.sets)||0),0);
    const exs=r.exercises.map((e,_ei,_arr)=>`<div class="exrow"><div class="exicon" style="background:${MC[e.muscle]||'#ccc'}18;border:1px solid ${MC[e.muscle]||'#ccc'}30">${exIcon(e)}</div><div><div class="exname">${esc(e.name)}</div><div class="exmet">${esc(e.muscle)} · ${esc(e.type)} · ${_coIco('timer',11,'⏱')}${restForExercise(e,r)}s${bisetInfo(_arr,_ei).biset?' · <span class="biset-tag">'+_coIco('link',10,'🔗')+' biserie</span>':''}</div></div><div class="exsets">${exSetsCellHTML(e)}</div></div>`).join('');
