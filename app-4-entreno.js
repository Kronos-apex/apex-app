...
function renderClientProfile(client){
  ...
  _pc.innerHTML=`<div class="profav tap" onclick="openAvatarPicker()" title="${_tieneAv?'Cambiar foto':'Agregar foto'}">${avInner}<div class="profav-cam">${typeof aviIcon==='function'?aviIcon('camera',12):'📷'}</div></div><div><div class="profname">${esc(client.name)}</div><div class="profmeta">${esc(client.email)}</div><div class="profpills">${client.goal?`<span class="profpill">${_pfi('target','🎯')} ${esc(client.goal)}</span>`:''}${client.level?`<span class="profpill">${_pfi('chart','📊')} ${esc(client.level)}</span>`:''}<span class="profpill">${_pfi('calendar','📅')} ${esc(String(client.days||3))} días/sem</span>${currentKg?`<span class="profpill">${_pfi('scale','⚖️')} ${currentKg} kg</span>`:''}</div><div class="profnote">Tu foto puede salir en las imágenes que compartes al terminar un entreno o un logro.</div>${_tieneAv?`<div class="profrm" onclick="removeAvatar()">✕ Quitar foto</div>`:''}</div></div>`;
...
function showWorkoutFinish(routine,stats){
  ...
  if(kcal!=null&&kcal>0)chips.push(['Calorías',`${fmtMiles(kcal)} kcal`]);
...
function _sessionSummary(s,reps,exCount,pct){
  ...
  if(s.kcal)t+=`${s.totalVol>0?' y':'Quemaste'} unas <b>${fmtMiles(s.kcal)} kcal</b>`;
...
function renderCoachClientHistory(clientId){
  ...
  _shown.forEach(s=>{
    ...
    const meta=[s.durationSec?fmtDuration(s.durationSec):'',s.kcal?`${fmtMiles(s.kcal)} kcal`:''].filter(Boolean).join(' · ');
