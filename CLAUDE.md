...
*Última actualización: 2026-07-26 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v399** (edge `refresh_snapshot` **v6**) · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **439/439** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` (`_verify-water` = agua+pasos, `_verify-news` = tour de novedades, `_verify-chatunified` = chat unificado + invitar a abrir la app, `_shot-trained` = ya entrenaste hoy, `_fable-repro-midsession` = regresión tarjeta a media sesión, `_verify-missday` = día que se corrió, `_verify-selftraining` = mi entrenamiento del coach, `_verify-share` = comparte AVI, `_verify-community` = comunidad C3+C5 + A1 prueba social (17, +CM13 bloqueo oculto en el gym, CM14-CM17 A1), `_verify-cmtynudge` = A2 puerta a Comunidad desde «Hoy» (13), `_verify-gyminvite` = A3 el coach invita al gym por WhatsApp (11), `_verify-milestoneask` = A4 opt-in de logros en el hito (19, +M9-M11 sesión típica/0 filas), `_verify-dm` = comunidad v2 ① chat en vivo (22), `_verify-lastactive` = comunidad v2 ② última conexión (14), `_verify-public` = ③c-2 cuenta pública (10), `_verify-follow` = ③c-3 descubrir+seguir (11), `_verify-feed` = comunidad v2 ④ muro/feed + re-forma + R2 hitos + entreno terminado (32), `_verify-workoutshare` = compartir entreno (8), `_verify-reports` = bandeja de reportes del coach (11), `_verify-comments` = comentarios del muro v3-a #4 (17), `_repro-cmty-identity` = la identidad de Comunidad NO se hereda entre cuentas (6, P0)) · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
> lote de adopción A1-A4 está EN PRODUCCIÓN pero **RECHAZADO** por la verificación (Opus se
> auto-verificó con el método de Fable: 4 revisores adversariales + sabotaje contra prod + repro en
> navegador). **✅ P0 CERRADO en avi-v398 · F2+F11 en avi-v399** — la identidad de Comunidad ya no se hereda entre cuentas
> (el bug que reportó el PO); cierra también **F1** (la sonda de A2 filtraba caras y apodos entre
> cuentas). **Sigue pendiente, en este orden:** F3 (A1 se
> desarma si alguien del gym se hace público — y hay que corregir el test que consagra el error),
> F4-F6 (modal del gym), F7-F13, F14 (`waPhone` manda la invitación a **Malasia** con un fijo de
> Bogotá) y §P3 (huecos de los harnesses). Sigue pendiente el veredicto vinculante de Fable.
