...
*Última actualización: 2026-07-17 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v364** · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **385/385** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` (`_verify-water` = agua+pasos, `_verify-news` = tour de novedades, `_verify-chatunified` = chat unificado + invitar a abrir la app) · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
*Adopción push ítem (c) (v364, 2026-07-17): botón 🔔 «Invitar a abrir la app» en la barra de `#coach-chat` → `coachInviteOpenApp()` → WhatsApp si hay teléfono (el chat interno solo pushea a suscritos; WhatsApp alcanza al no-suscrito), si no prellena `#cchat-in` (el coach envía). +7 checks en `_verify-chatunified.mjs`. Falta verificación de Fable.*
