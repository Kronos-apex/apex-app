...
### Coach Inteligente (v352 — "alguien pendiente de ti")
...
- `coachInsight(client, sessions, prs, now, opts)` (avi-core) — función PURA, devuelve el insight priorizado o null. Prioridad: **inactivo(≥4d) > récord(PR ≤48h) > racha(≥2 semanas de plan) > estancado(kg, ≥6 puntos planos, SOLO premium) > adaptación**. Umbrales = constantes `INSIGHT_*`. Recibe `now` siempre (determinista). `renderCoachCard` (app-4) la pinta en `#cn-coach-card` (Hoy). "Entendido" silencia por 2-7 días (`coachmute_<cid>_<type>` localStorage, LOCAL — NO en SB_KEYS a propósito).
...
- [~] 🧠 **COACH INTELIGENTE ("alguien pendiente de ti") → plan vivo en `docs/plan-coach-inteligente.md`** (idea de Camilo 2026-07-13). **Fases 1+2 EN PROD (avi-v352, 2026-07-15):** Capa A = `applyMood` devuelve `adapt.care` (1-3 consejos de bienestar por ánimo, voz AVI, nunca médicos) → bloque "Para cuidarte hoy" en `moodBannerHtml`; Capa B = motor puro `coachInsight` (récord/racha/inactividad/estancamiento/adaptación, prioridad + mute + free/premium gating) → tarjeta `#cn-coach-card` en Hoy. Decisiones de Camilo cerradas (§9 del plan): voz AVI cálida, gancho free/fino premium, sin push por ahora, coach-para-el-coach en Fase 3. **Falta:** Fase 3 (deload/hábitos/peso, coach-para-el-coach, push cuando suba la adopción) y la capa LLM opcional (futuro). 📷 Análisis de técnica por CÁMARA = apuesta futura SEPARADA (rompe single-file/offline, riesgo legal; escalón intermedio = grabar video para el coach).
...
*Última actualización: 2026-07-15 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v352** · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **327/327** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*

*Coach Inteligente Fases 1+2 EN PROD (v352, 2026-07-15): Capa A `adapt.care` (bienestar por ánimo) + Capa B motor puro `coachInsight` + tarjeta `#cn-coach-card` en Hoy. Falta Fase 3 (deload/hábitos/peso, coach-para-el-coach, push) y capa LLM opcional. Plan+verificación en `docs/plan-coach-inteligente.md`.*
