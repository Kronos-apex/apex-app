...
*Última actualización: 2026-06-10 · Marca: **AVI** · **v2.0 (auth real + RLS, EN PRODUCCIÓN)** · **apex-v120** · Suite **123/123** verde · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
**🎬 VIDEOS — 1 video por ejercicio para TODOS** (el sexo NO define el video; Camilo 2026-06-09; hacer 220 es inviable). `exVidSrc(e)` sin sexo. En prod: e1,e2,e5,e6,e9,e11,e13,e22,e23,e24,e27,e36,e84 + `EX_VID_F` (archivo `_f`, se muestra a todos) e33,e34,e37,e58. **PENDIENTE pasada limpia: e80 (sentadilla ♀) + e42 (hip thrust ♀)** — clips en Descargas. Lección: las reps de Veo tienen timing irregular → ubicar la ventana con la tira del **CLIP final** (no del source), `-ss` EXACTO (después de `-i`), 1 rep con endpoints iguales, 720²/blurred-fit, grade, sin audio. Ver `project_avi_video_pipeline_gender`.

**📸 FOTOS (NECESITA GEMINI).** ⭐ Flujo: **Camilo renombra el archivo con el nombre del ejercicio**, Claude lo mapea al catálogo (texto) y procesa (delogo ✦ + 720² + grade Noir). **Claude NO identifica visualmente** (es malo, ver `feedback_avi_imagenes_verificar_contenido`). Sin foto nuestra (11): e45, e72, e74, e75, e76, e90, e91, e108, e109 + e89♂, e93♂. **Ocultas en `EX_IMG_HIDE` (foto incorrecta): e92, e31, e105.** Mala (no muestra el jalón, regenerar): **e4**. *(Opcional: ~30 fotos STOCK gym rojo/azul → reemplazar por AVI.)* Para nuevas-sin-entrada en `EX_IMG_NAME`, añadir el id a `EX_IMG_IDS` (ver e63).
...
**Otras pendientes:** ampliar glosario (Valsalva/tempo). Datos prod de Miguel: ver memorias.
---
