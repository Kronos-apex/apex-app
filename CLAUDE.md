...
*Última actualización: 2026-06-29 · Marca: **AVI** · **v2.0 (auth real + RLS, EN PRODUCCIÓN)** · **avi-v229** · Catálogo **212 ejercicios** (e1–e214; +50 nuevos e165–e214: movilidad, HIIT/funcional, antebrazo/grip) + **gate por nivel** en el generador · Suite **255/255** verde · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*

*Tamaño de texto + biserie (v229): el ajuste de Perfil escala con `zoom` (px fijos). BUG: la compensación de ancho (`width:calc(100%/zoom)` + `transform-origin:left top`) estaba SOLO en `.cnp` → el Perfil del coach (`.panel`) y el modo guiado (`.gm-body`) se DESBORDABAN (correo cortado, botones fuera de la tarjeta) y otras pantallas se encogían a la izquierda con hueco a la derecha. FIX: compensación PAREJA en `.cnp`/`.panel`/`.gm-body` + factores más grandes (lg 1.12→1.18, xl 1.25→1.40). Además el indicador de biserie 🔗 (morado) heredaba los 11px de `.exmet` → ilegible en plena rutina; ahora clase `.biset-tag` (13.5px, peso 800). 🟡 falta confirmar en el celular de Camilo (esta feature ha tenido idas y vueltas por el `zoom` del WebView).*
