...
- 🔴 **Todo estado en MEMORIA o en localStorage que dependa de QUIÉN inició sesión debe limpiarse en `logout()` — y llevar el uid en el nombre de la clave.** `logout()` limpia `ax_session` y `_pushCtx` (y comenta que lo hace por esta clase), pero **NO recarga la página** y no toca el objeto `CMTY`: en todo el repo no existe ni un `CMTY.profile=null` / `CMTY.uid=null` / `CMTY.loaded=false`. Como `renderCommunity()` corta con `if(!CMTY.loaded) cmtyLoad()`, la 2ª cuenta de la misma pestaña **no recarga y ve los datos de la 1ª**: perfil, código, amigos y la **bandeja de DMs**. Reportado por el PO (2026-07-25: «en el perfil de Astrid aparecía el mío») y REPRODUCIDO en `scripts/e2e/_repro-cmty-identity.mjs`. Gemelo en disco: `ax_cmty_cache`/`ax_cmty_probe`/`ax_cmtynudge` son claves GLOBALES del dispositivo con datos de terceros y sobreviven al logout Y a la recarga. **Regla: clave local con datos de otra persona → nombre con uid (`ax_cmty_msask_<uid>` lo hace bien) + borrado en `logout()`; y todo objeto-módulo con identidad → función de reset invocada desde `logout()`, más un candado que compare el uid resuelto contra el guardado.** Plan de corrección: `docs/plan-correcciones-adopcion.md` §P0.
...
*Última actualización: 2026-07-25 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v397** (edge `refresh_snapshot` **v6**) · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **437/437** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` (`_verify-water` = agua+pasos, `_verify-news` = tour de novedades, `_verify-chatunified` = chat unificado + invitar a abrir la app, `_shot-trained` = ya entrenaste hoy, `_fable-repro-midsession` = regresión tarjeta a media sesión, `_verify-missday` = día que se corrió, `_verify-selftraining` = mi entrenamiento del coach, `_verify-share` = comparte AVI, `_verify-community` = comunidad C3+C5 + A1 prueba social (17, +CM13 bloqueo oculto en el gym, CM14-CM17 A1), `_verify-cmtynudge` = A2 puerta a Comunidad desde «Hoy» (13), `_verify-gyminvite` = A3 el coach invita al gym por WhatsApp (11), `_verify-milestoneask` = A4 opt-in de logros en el hito (13), `_verify-dm` = comunidad v2 ① chat en vivo (22), `_verify-lastactive` = comunidad v2 ② última conexión (14), `_verify-public` = ③c-2 cuenta pública (10), `_verify-follow` = ③c-3 descubrir+seguir (11), `_verify-feed` = comunidad v2 ④ muro/feed + re-forma + R2 hitos + entreno terminado (32), `_verify-workoutshare` = compartir entreno (8), `_verify-reports` = bandeja de reportes del coach (11), `_verify-comments` = comentarios del muro v3-a #4 (17)) · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
> 🚨 **PRIORIDAD AL RETOMAR (2026-07-25) → `docs/plan-correcciones-adopcion.md`.** El lote de
> adopción A1-A4 está EN PRODUCCIÓN pero **RECHAZADO** por la verificación (Opus se auto-verificó
> con el método de Fable: 4 revisores adversariales + sabotaje contra prod + repro en navegador).
> **P0 = bug reportado por el PO y REPRODUCIDO:** la identidad de Comunidad queda PEGADA entre
> cuentas en la misma pestaña (Camilo vio SU perfil dentro de la pantalla de Astrid; también
> arrastra la bandeja de DMs). Preexistente del módulo C3, ampliado por A2. Luego: A4 no se pinta
> en la sesión típica, A1 se desarma si alguien del gym se hace público, y 3 defectos del modal del
> gym. **Nada de esto está corregido.** Sigue pendiente además el veredicto vinculante de Fable.
