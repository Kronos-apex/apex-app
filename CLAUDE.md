...
**FASE B — VIDEOS (NECESITA GEMINI).** En prod ♂/único (9): e5,e6,e9,e11,e13,e22,e27,e84,**e36**. **♀ gender-aware (`EX_VID_F`): e33,e34,e37,e58** (nuevo sistema, ver `project_avi_video_pipeline_gender`). Pendiente: **regenerar e28** (logo roto) + más compuestos (e1 press banca, e4 dominadas, e7 press militar barra, e42 hip thrust…). Receta: contact-sheet→identificar→recorte -ss→~5s loop→720²/blurred-fit→grade→sin audio. **REGLA: revisar el LOGO de cada video.**

**FASE C — fichas premium (SIN Gemini, pendiente):** nombres específicos + desc que nombre el músculo. ⚠️ rename necesita **alias en `EX_IMG_NAME`** para no romper rutinas guardadas por nombre.
