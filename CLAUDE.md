...
*Tamaño de texto — ✅ RESUELTO (v233, CONFIRMADO por Camilo en PC + celular): el ajuste de Perfil escala con `zoom` (px fijos), y el `zoom` se comporta OPUESTO según el motor — estandarizado (Chrome/WebView ≥128, escritorio) NO escala el ancho (compensar deja hueco a la derecha); legacy (Huawei sin Google) SÍ lo escala (sin compensar desborda y corta). Ningún CSS fijo sirve para ambos → se DETECTA en runtime: `detectZoomWidthScaling()` (app-1-infra.js, desde initTextSize) mide un hijo `width:100%;zoom:2` en un padre fijo de 300px → ratio>1.5 = legacy → marca `html[data-zoomw="scale"]`. El `zoom` (factores lg 1.18 / xl 1.40) va SIEMPRE; la compensación `width:calc(100%/zoom)`+`transform-origin:left top`+`max-height:calc(/zoom)`+anclaje del `.mdbg` van SOLO bajo `[data-zoomw="scale"]`. Verificado headless en escritorio (sin comp llena 100%) y por Camilo en su Huawei. **REGLA: la compensación NO es global — va gated por detección; nunca hardcodear "siempre con/sin comp".** Biserie 🔗 morado: clase `.biset-tag` (13.5px, peso 800; antes heredaba 11px de `.exmet`).*

*Caché del WebView + cache-bust (v230→v234): el `styles.css` estaba **cache-first** en el SW → tras un cambio de estilos el celular (sobre todo Huawei, cuyo WebView guarda el caché POR FUERA de la app → ni desinstalar lo borra) servía el CSS viejo ("agrandé y sigue igual"). FIX: `styles.css` ahora **network-first** como el JS (sw.js), y los assets en index.html llevan **`?v=NNN`** (cache-bust por URL: archivo "nuevo" = salta cualquier caché aunque el SW viejo siga activo). **Al tocar CSS/JS, subir el `?v=NNN` en index.html + el `CACHE_NAME` del sw.js.** Imágenes nuevas (v234): los 50 ids e165–e214 faltaban en `EX_IMG_IDS` → no se mostraban aunque el `.jpg` existía (misma trampa que e97/e126); registrados.*
...
*Navegación ATRÁS (Android/TWA) — avi-v223: cada habitación (.sroom) empuja su propia entrada de historial vía `navOpenLayer` (AVINAV.layers); el handler `_aviHandleBack` cierra la capa de arriba y descuenta SIN re-empujar (antes re-empujaba un único guard dentro del popstate, que el WebView del TWA perdía → la app se salía al 2º atrás). Los botones "‹ Volver" pasan por `history.back()`. **+avi-v243 (Clase 3 auditoría 2026-06-30):** la ficha técnica (`#exdetail-bg`) y el lightbox (`#ex-lightbox`) TAMBIÉN empujan capa propia (antes se abrían sobre una sala sin capa y el paso 0 les descontaba la capa de la SALA → el siguiente atrás caía en la op frágil y el TWA podía cerrar la app — #7); sus cierres por UI (tap fondo, swipe-down, tap lightbox, Escape) pasan por `navCloseLayer(closeFn)` → `history.back()` (colchón: cierre directo si layers=0). REGLA: todo overlay que pueda abrirse ENCIMA de una .sroom debe empujar capa propia; nunca cerrar por UI con la función directa si empujó capa. "← Volver" del detalle del coach = `history.back()` (#11, antes gp silencioso dejaba un atrás muerto). `_aviInstallBack()` (guard+handler, una sola vez) corre en boot feliz Y en el catch. Harness: `Desktop/AVI/_repro-back-v243.mjs` (17/17; su freshLogin evita el doble-doLogin que hacía flaky a `_repro-back.mjs`).*
...
1. **⭐ `#premium-upsell` — "Quiero un coach" full-bleed** (commit `c2b66a4`): la venta pasa de cajita inline a momento inmersivo — foto `media/brand/ob-3.jpg` (override `window.AVI_UPSELL_PHOTO`) + eyebrow oro "AVI Premium" + 4 beneficios (plan/técnica/chat/seguimiento) + CTA. Los CTAs existentes de `coachUpsellHTML` y `premiumLockHTML` ahora abren el overlay (`showPremiumUpsell`, guard libre/selfReg) y la solicitud se confirma ADENTRO (`puConfirm` → `requestCoach()` intacto → estado "Solicitud enviada" + confetti). Si ya pidió coach, el overlay abre directo en el estado enviado.
2. **🆙 `#level-up` — celebración al subir de nivel** (mismo commit): detectada en `showWorkoutFinish` (el historial YA incluye la sesión en ambos caminos: `updateClientProgress` guarda antes; el guiado tiene auto-guardado parcial desde la 1ª serie) → `gxLevel(tot)` vs `gxLevel(tot-1)` + guard `localStorage['gx_lvlup_{clientId}']` (no re-celebra ni dispara para niveles ya alcanzados históricamente). Se **encadena al cerrar** el fin de entreno (no compite con él). Foto `media/brand/reveal.jpg` (override `window.AVI_LEVELUP_PHOTO`) + anillo con número Anton + nombre del nivel + siguiente meta.
3. **🔙 Botón atrás** cierra los 3 momentos full-bleed (`premium-upsell`/`level-up`/`workout-finish` añadidos al handler popstate; antes `workout-finish` tampoco estaba).
4. **🧹 CSS muerto `.apex-logo` eliminado** (12 reglas huérfanas del logo "A" quitado en v56; commit `9dffedf`, sin bump SW — navigate es network-first).
- **Decisión de diseño:** "descanso entre series" NO se hace full-bleed (uso diario constante — diluye el efecto y cargaría una foto en plena serie; contradice el principio "solo MOMENTOS").
...
**🎨 REDISEÑO INTERIOR — ✅ COMPLETO (apex-v119).** Diseño final pre-lanzamiento, todo al nivel del onboarding premium. Hecho: tema dark default · **Hoy** (hero foto + **rutina del día en ORO** en Rutinas) · **ficha** (foto full-bleed + glosario + respiración + nota coach) · **check-in** premium (`#data-ob`) · **Progreso consolidado** (pestaña "Historial"→"Progreso": nivel+descuento+gráfica+sesiones `.sescard` repulidas; `#cn-gamif` movido de Perfil) · **Perfil/seguimiento** premium (`.prof-collapse` + `#cn-profile .ctitle` Anton esmeralda, scoped). Skills: `frontend-design` + `frontend-design-review`. Preview: `_preview-interior.html` (6 pantallas).


**🎬 VIDEOS — 1 video por ejercicio para TODOS** (el sexo NO define el video). 🚫 **REGLA (2026-06-10): Camilo edita/recorta los videos ÉL MISMO y me los pasa listos; Claude NO recorta ni grade — solo enchufa** (las ediciones de Claude salían con mala técnica). Mi rol = recibir el archivo final → 720²/blurred-fit + quitar audio + registrar + bump SW (NO re-editar). En prod (~28 ejercicios): `EX_VID`=e1,e2,e3,e5,e6,e9,e11,e13,e14,e22,e23,e24,e26,e27,e28,e36,e37,e42,e51,e52,e56,e58,e80,e84,e85,e86 · `EX_VID_F`=e34 · **`EX_VID_BOTH`=e2,e33** (gender-aware: existen `.mp4` ♂ + `_f.mp4` ♀ → `exVidSrc` muestra la del sexo del usuario; Camilo: "muestra ambos"). Sin videos pendientes por nombrar. Ver `project_avi_video_pipeline_gender`.

**📸 FOTOS — ✅ 110/110 CON FOTO, `EX_IMG_HIDE` VACÍO (2026-06-10).** Flujo que funcionó: **Camilo nombra el archivo por ejercicio**, Claude mapea y procesa con `scripts/process-photos.sh` (720² blurred-fit, sin recortar técnica). Las de Gemini ya vienen con el look AVI → **sin grade ni delogo**. **Claude NO identifica visualmente** (`feedback_avi_imagenes_verificar_contenido`). Para nuevas-sin-entrada en `EX_IMG_NAME`, añadir el id al array de `EX_IMG_IDS`. (e4 dominadas: revisar si es dead-hang, menor.)

**🖼️ FOTOS STOCK POR REEMPLAZAR — quedan 22 de 46 (24 ya reemplazadas 2026-06-10).** Detección por tamaño: las AVI son **720²**, las stock viejas **320×213/320×320**. 88 ya son AVI. Lista actualizada en **`Desktop/FOTOS-STOCK-POR-REEMPLAZAR.txt`**. Pendientes: e20,e32,e37,e39,e46,e49,e59,e60,e61,e64,e65,e66,e67,e68,e69,e70,e81,e87,e95,e96,e106,e107. Camilo las regenera en Gemini → me las pasa nombradas → proceso e integro (mismo flujo).

**FASE C — fichas premium: ✅ EFECTIVAMENTE COMPLETA (auditada 2026-06-10).** Las 109 fichas tienen `descSimple`+`muscleLabel`+`coachTip`+`ytQuery` a nivel premium (lo que ve el asesorado). Lo único flojo son ~27 `desc` técnicos de una línea, pero ese campo **solo se ve en la tarjeta de biblioteca del coach (recortado a 80 chars) y en el editor** — el asesorado NUNCA lo ve (la ficha usa `descSimple`; el guiado usa `breathCue`). Pulirlos = cosmético coach-only, valor casi nulo. NO priorizar.
