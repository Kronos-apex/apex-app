...
*Última actualización: 2026-06-10 · Marca: **AVI** · **v2.0 (auth real + RLS, EN PRODUCCIÓN)** · **apex-v127** · Suite **123/123** verde · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
**🎬 VIDEOS — 1 video por ejercicio para TODOS** (el sexo NO define el video). 🚫 **REGLA (2026-06-10): Camilo edita/recorta los videos ÉL MISMO y me los pasa listos; Claude NO recorta ni grade — solo enchufa** (las ediciones de Claude salían con mala técnica). Mi rol = recibir el archivo final → 720²/blurred-fit + quitar audio + registrar `EX_VID`/`EX_VID_F` + bump SW (NO re-editar). En prod (24 ejercicios): `EX_VID`=e1,e2,e5,e6,e9,e11,e13,e14,e22,e23,e24,e26,e27,e28,e36,e37,e42,e51,e52,e58,e80,e84 + `EX_VID_F`=e33,e34. **PENDIENTE: nombrar 4 videos nuevos en `Descargas/VIDEOS LISTOS`** — `APERTURAS EN POLEA` (¿e85 alta o e86 baja?) + 3 sin nombre (`Keep_the_person (14)`, `VID_…092201`, `VID_…092202`). e2 tiene versión ♂ (en prod) y ♀ (`e2_f.mp4` en disco, no se muestra) — decisión pendiente. Ver `project_avi_video_pipeline_gender`.

**📸 FOTOS — ✅ 110/110 CON FOTO (2026-06-10).** Flujo que funcionó: **Camilo nombra el archivo por ejercicio**, Claude mapea al catálogo y procesa con `scripts/process-photos.sh` (720² blurred-fit, sin recortar técnica). Las de Gemini ya vienen con el look AVI horneado → **sin grade ni delogo**. **Claude NO identifica visualmente** (`feedback_avi_imagenes_verificar_contenido`). Único pendiente: **e105** (Ext. tríceps con banda) sigue oculto en `EX_IMG_HIDE` (foto incorrecta, no llegó la correcta) + **e4** (dominadas, revisar si es dead-hang). 1 imagen sin nombre en `Descargas/IMAGENES LISTAS` (`Gemini_Generated_…`) → renombrar. Para nuevas-sin-entrada en `EX_IMG_NAME`, añadir el id al array de `EX_IMG_IDS`.
...
**Otras pendientes:** nombrar 4 videos + 1 imagen nuevos (arriba); foto correcta de e105; revisar e4; ampliar glosario (Valsalva/tempo). Datos prod de Miguel: ver memorias.
---
