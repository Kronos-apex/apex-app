...
- [~] 🔔 Asesorados: subir las suscripciones a push (mecánica ya arreglada en v320: toast honesto + self-heal + denied con instrucciones). ADOPCIÓN (producto): (b) pedir permiso al FIN del entreno **YA EXISTE (v325, `renderWfPushNudge`)** — verificado en la Sesión J. Quedan (a) copy más fuerte de la tarjeta de "Hoy", (c) que el coach invite por chat a abrir la app. El iPhone real sin PWA instalada no entra hasta que la instale (Lucas v320)
...
*Última actualización: 2026-07-17 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v363** · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **385/385** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` (`_verify-water` = agua+pasos, `_verify-news` = tour de novedades, `_verify-chatunified` = chat unificado del coach) · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
*Chat unificado del coach (SESIÓN K, v363, 2026-07-17): la ficha `p-detail` muestra un PREVIEW de solo lectura (últimos 2 mensajes + «Abrir chat») en vez del input inline; `sendCoachMsg`/`#msg-in` borrados. Un solo lugar para escribir = el chat de pantalla completa `#coach-chat` (v321). Harness `_verify-chatunified.mjs`. Falta verificación de Fable.*
