...
*Última actualización: 2026-07-26 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v400** (edge `refresh_snapshot` **v6**) · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **441/441** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` (`_verify-water` = agua+pasos, `_verify-news` = tour de novedades, `_verify-chatunified` = chat unificado + invitar a abrir la app, `_shot-trained` = ya entrenaste hoy, `_fable-repro-midsession` = regresión tarjeta a media sesión, `_verify-missday` = día que se corrió, `_verify-selftraining` = mi entrenamiento del coach, `_verify-share` = comparte AVI, `_verify-community` = comunidad C3+C5 + A1 prueba social (19, +CM18 pertenencia real al gym), `_verify-cmtynudge` = A2 puerta a Comunidad desde «Hoy» (13), `_verify-gyminvite` = A3 el coach invita al gym por WhatsApp (16, +F4/F5/F6), `_verify-milestoneask` = A4 opt-in de logros en el hito (19, +M9-M11 sesión típica/0 filas), `_verify-dm` = comunidad v2 ① chat en vivo (22), `_verify-lastactive` = comunidad v2 ② última conexión (14), `_verify-public` = ③c-2 cuenta pública (10), `_verify-follow` = ③c-3 descubrir+seguir (11), `_verify-feed` = comunidad v2 ④ muro/feed + re-forma + R2 hitos + entreno terminado (32), `_verify-workoutshare` = compartir entreno (8), `_verify-reports` = bandeja de reportes del coach (11), `_verify-comments` = comentarios del muro v3-a #4 (17), `_repro-cmty-identity` = la identidad de Comunidad NO se hereda entre cuentas (6, P0)) · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*
...
> 🚨 **PRIORIDAD AL RETOMAR (actualizado 2026-07-26) → `docs/plan-correcciones-adopcion.md`.** El
> lote de adopción A1-A4 está EN PRODUCCIÓN pero fue **RECHAZADO** por la verificación. **YA
> CORREGIDOS:** P0+F1 (avi-v398, identidad de Comunidad pegada entre cuentas) · F2+F11 (avi-v399, la
> pregunta de logros era inerte en la sesión típica y confirmaba sin publicar) · F3+F4-F6 (avi-v400,
> la pertenencia al gym la dice el servidor vía `cmty_gym_peers`, y el modal del gym ya no miente).
> **Sigue pendiente, en este orden:** F7 (la puerta de A2 no se cierra al cruzarla) · F8 (sonda
> mentirosa por fallo parcial) · F9 (sonda corrupta rompe «Hoy») · F10 (un «Sí» sin conexión quema
> la pregunta) · F12 (ignorar la tarjeta no la calla) · F13 (la pantalla de fin no scrollea) ·
> **F14** (`waPhone`: un fijo de Bogotá manda la invitación a **Malasia**) · §P3 (huecos de los
> harnesses). Sigue pendiente el veredicto vinculante de Fable sobre todo el bloque.
