...
*Última actualización: 2026-07-15 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v354** · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **352/352** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*

*Coach Inteligente Fases 1+2+3+4 EN PROD (v354, 2026-07-15): Capa A `adapt.care` + Capa B `coachInsight` (8 señales, `#cn-coach-card` en Hoy) + Pulso del coach `coachPulse` (`#h-pulse` en Inicio) + **PLAN DE CHOQUE** `shockPlan`/`applyShockOption` (`#d-shock` en la ficha: AVI propone el protocolo contra el estancamiento, el coach lo aplica en 1 toque y le escribe). Falta capa LLM opcional y push (futuro). Plan+verificación en `docs/plan-coach-inteligente.md`.*
