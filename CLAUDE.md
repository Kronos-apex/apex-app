...
- ⚠️ **El repo va con finales de línea MIXTOS y un patrón de sabotaje con el salto equivocado NO SE APLICA NUNCA.** Medido el 2026-08-12: `avi-core.js` va con **CRLF** (6.917) y `app-5-salud.js` con **LF** (1.904, cero CRLF) — la versión anterior de este gotcha decía «los archivos de este repo van con CRLF» y es FALSO en al menos un módulo de pantalla, lo que hizo que el sabotaje 15 de F7 no se aplicara. Pasó antes en `_sabotaje-f6.mjs`, y el runner lo reporta como «NO SE APLICÓ» — pero es un renglón que se lee de reojo igual que un ✅, y en un listado de 17 se pierde. **Los patrones se anclan en UNA sola línea**, el runner tiene que gritar cuando el texto no aparece exactamente 1 vez, y si hace falta multilínea, se comprueba el final de línea DE ESE archivo antes de escribirlo.
