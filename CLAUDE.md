...
- [ ] 📧 **Correo de confirmación de marca** (caso real 2026-07-12: un usuario vio el enlace crudo larguísimo de Supabase, le dio MIEDO y NO abrió la app). Plantilla premium lista en `docs/email-templates/confirm-signup.html` (botón grande, tono Sofía, copy que tranquiliza). **PENDIENTE Camilo: aplicarla en Supabase → Auth → Email Templates → "Confirm signup"** (pasos en `docs/email-templates/README.md`). TODO: mismo tratamiento a las otras plantillas (magic link, reset, invite). Opción fuerte a futuro: código OTP de 6 dígitos en vez de enlace (elimina el miedo de raíz).
...
*Última actualización: 2026-07-12 · Marca: **AVI** · **v2.x (auth real + RLS + guiado único, EN PRODUCCIÓN)** · **avi-v330** · Catálogo **212 ejercicios** (e1–e214, todos con foto) · Suite **312/312** verde · QA: hook 11 checks (`scripts/hooks/`, `core.hooksPath`) + CI + harnesses `scripts/e2e/` · repo local: `Desktop/AVI/apex-app` · Tagline: "Entrenamiento con nombre propio" · PO: Camilo Andrés*

*Hitos por sesión: **`docs/bitacora.md`** (los 3 más recientes: **v330 elevación PREMIUM FASE 0 #2** (sistema de elevación de 3 niveles; borde de tarjeta `--br`→`--br2` en `.card`/`.cli`/`.exc`, arregla el H5 de separación débil en claro; decidido por preview de variantes A/B/C en ambos temas) · **v329 elevación PREMIUM FASE 0 #1** (tokens de movimiento `--ease-out`/`--dur` + press-feedback en `.btn`/`.cli`) · **v328 re-barrido XSS**).*
