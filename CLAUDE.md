...
- [ ] Supabase Auth password hardening — **descubierto 2026-07-13: leaked-password protection es SOLO plan Pro (US$25/mes; la org está en Free) → NO es un toggle pendiente, es una decisión de plan (diferida — no vale Pro solo por esto).** Lo accionable en Free: requisitos server-side de contraseña en Auth → Providers → Email (min 8 + "Lowercase, uppercase letters and digits", espejo EXACTO de `passwordProblem` en avi-core — si el server exigiera más que el cliente, el usuario vería el error crudo de Supabase en vez del mensaje Sofía). Camilo lo estaba configurando 2026-07-13.
